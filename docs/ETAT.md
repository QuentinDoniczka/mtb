...
- **#34** — déplacer la feuille de la galerie vers le thème (T15) · `includes/blocks/galerie-photos/{galerie.css,block.json}` · `mtb-core/bootstrap.php` · `themes/mtb/assets/css/blocs/mtb-galerie-photos.css`. **Doit tourner seule**, sa propre note d'exécution l'exige.
...
**Questions en attente pour l'utilisateur, ouvertes au lot 14** :
1. **T89 — faut-il inscrire l'obligation de `make css` au § Conventions de `CLAUDE.md` ?** C'est le coût
   permanent introduit par #40 et il n'a aujourd'hui aucun domicile qu'une chaîne lise avant d'éditer une
   feuille. Je n'ai pas modifié `CLAUDE.md` de ma propre initiative.
2. **Une capture reste à prendre** : `captures/portee-mention-photo-principale.png` — l'encadré jaune en
   tête de « Galerie photos ». Déclarée manquante par #36, **aucune référence orpheline ajoutée**,
   bijection du guide vérifiée intacte.
3. **T81 est résolue et n'appelle pas de question** : l'absence de photo principale sur les 27 portées
...
| **T85** | **`MTB_CORE_VERSION` est figé à `0.1.0`** : un `editeur.js` modifié garde son URL. Sans effet sur le livrable du lot 14, mais vaut pour les **onze** `editeur.js` de l'extension | lot 14, #43 | une issue `infra` |
