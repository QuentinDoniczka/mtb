...
**Phase : lot 22, seconde partie (#57 seule, T114) livrée, testée et revue le 2026-09-18.** Deux
commits, `5bbf54b` (code) et `4d23c28` (levée du HIGH de revue). Intégration **54/54, 0 échec** ; quatre
défauts du harnais corrigés en cours de route, aucun du code. Revue **BLOQUANTE au premier passage —
0 CRITICAL, 1 HIGH** ; levée, puis **H1 LEVÉ**. Pile Docker vérifiée à volume conservé : build ✓,
boot ✓, `provision.sh` rejoué (0 créé / 14 présents / 0 rejeté), journaux propres, et
`ANCRES DU CŒUR : ok — 77 fichiers du cœur cités avec un numéro de ligne, tous identiques au relevé ;
WordPress 6.9 installé.`

**Ce que le lot rend à l'éleveuse : rien de visible, et c'est exact** — c'est de la dette d'outillage.
Le code exécutable des sept fichiers PHP touchés est **identique avant et après**, vérifié au
tokenizer : le commit n'ajoute que des commentaires datés.

**L'issue demandait de surveiller un numéro de version ; la chaîne a surveillé le fait réel, et c'est
mieux.** Le témoin compare l'**empreinte md5 des 77 fichiers du cœur** cités quelque part avec un
numéro de ligne, pas la version. Conséquence : une 6.9.x qui ne touche aucun fichier cité ne déclenche
rien — **la fausse alerte qu'on apprend à ignorer n'est pas émise** (décision 83). La version ne sert
plus que de contexte. **Deux chiffres de l'énoncé étaient faux** : « trois fichiers et trois
contrats » vaut en réalité **77 occurrences dans 11 contrats**, et le périmètre est le défaut, pas la
liste de sept ancres (décision 78).

**Le HIGH est encore un faux vert de documentation, et cette fois il portait sur un lecteur qui
n'existait pas.** `issue-57.md:116` affirmait « `docker-mtb` relit ce journal en fin de lot » : aucune
consigne ne le demandait. **Un témoin dont la ligne n'est lue par personne ne remplace pas le
silence, il le déguise.** Corrigé par `4d23c28`, hors empreinte de la chaîne parce que ce sont des
prompts : étape 8 obligatoire dans `docker-mtb`, ligne recopiée telle quelle, absence de ligne valant
anomalie, relais par le lead et ouverture d'une relecture des ancres sur statut 20 ou 30. **Décision
87.** Deux précisions ajoutées après la première exécution réelle : attendre `terminé.` avant le
`grep`, le healthcheck `wpcli` passant `healthy` **~70 s avant la fin du provisionnement**, sinon
l'absence de ligne se lit à tort comme « témoin absent » ; et recopier le bilan, pas le rappel posté
après `terminé.`, qui porte le même mot-clé.

**Deux faits d'exploitation à ne pas perdre.** (1) **Les mises à jour automatiques du cœur sont
réellement actives dans la pile de développement** : `auto_update_core_major` est à `enabled`, le cron
`wp_version_check` est dû, le cœur est inscriptible. WordPress peut donc passer en 7.1 tout seul, et
l'ALERTE n'apparaîtrait qu'au provisionnement suivant — `wp-includes/version.php` étant au registre,
toute montée réelle lèvera une ALERTE. **Dette T126.** (2) Le commit décale les lignes de sept
fichiers PHP de 6 à 43 rangs : **trois citations d'`ETAT.md` sont devenues fausses**, corrigées ici
même. C'est T88, la dette de méthode, et le témoin ne couvre délibérément pas les fichiers du dépôt.

**Prochaine action : `/lead-mtb #58`, seule.** Elle écrit dans le même `indexation-heritee/bootstrap.php`
que #57 vient d'amender, et devra se conformer au critère d'entrée du module formulé par le défaut, qui
remplace la formule « périmètre clos et daté ». **#60 reste hors lot** tant que sa question bloquante
n'est pas tranchée et son empreinte pas écrite. **#26 et #48 restent gelées par Q5.**

**Lignes de DoD NON vérifiées par ce lot** : D1 à D5, D7, D8, D10, D12 — sans objet, aucun écran,
contenu ni rendu modifié, ce qui est plus fort que « non vérifiée ». **D6 couverte** (66 pages, une
seule origine, aucun cookie). **D9 couverte sauf le démarrage à froid**, toujours interdit par mes
bornes : la base porte le contenu repris. **Aucune vraie montée de version n'a été jouée** — elle est
simulée par variable et par empreinte altérée.

---

**Phase : lot 22, première partie (#59 seule, T118) livrée, testée et revue le 2026-09-17.** Deux
commits, `0379804` (code) et `1fcf948` (corrections de revue). Intégration **104/104** : deux échecs
au premier passage, imputables au test. Revue **BLOQUANTE au premier passage — 0 CRITICAL,
1 HIGH** ; levée par `1fcf948`, puis **OK avec réserves**. Pile Docker vérifiée à volume conservé :
build ✓, boot ✓, `provision.sh` rejoué deux fois (0 créé / 14 présents / 0 rejeté), module prouvé
par `wp eval`, `debug.log` sans ligne nouvelle.

**Ce que le lot rend à l'éleveuse** : dans la **Modification rapide** des listes Pages, Portées et
Chiens, le champ s'appelle « **Adresse de la page** » et non plus « Slug ». C'est le même mot que
sur les écrans complets.

**Le HIGH est une nouvelle famille de faux vert de documentation (décision 86).** La chaîne avait
cherché dans le guide le mot « Modification rapide », ne l'avait trouvé nulle part, et en avait
conclu qu'aucune fiche n'était fausse. Or la fiche chien affirmait « c'est le seul endroit où
l'adresse d'une fiche se change ». **Ajouter un chemin rend fausse la phrase qui niait qu'il
existe.**

**Le fait le plus lourd du lot n'est pas dans son code : T125.** Mesurée par la chaîne à ma
demande, en transaction annulée. **Changer ou vider l'adresse d'un chien ou d'une page coupe en
silence la 301 venue de l'ancien site**, parce que `redirections-301` retrouve ces cibles par leur
adresse. Les portées, retrouvées par leur identifiant, sont épargnées. C'est une brèche latente de
la contrainte 4, **antérieure à #59** : seule `wp mtb verifier-redirections` la voit. La fiche
chien avertit désormais l'éleveuse, aucune fiche page ne le fait. **D'où Q29, posée à
l'utilisateur** : masquer la Modification rapide à l'éleveuse (T123 : Mot de passe, Privée et État
y sont offerts sans explication) ?

**Prochaine action : lot 22, suite = `/lead-mtb #57` puis `/lead-mtb #58`, séquentielles.** Les
deux issues écrivent `indexation-heritee/bootstrap.php` : verdict de `github-boards` calculé sur le
disque. Aucune feuille CSS n'est touchée. **#57 doit amender par acte daté le motif gelé du module**
(voir plus bas). **#60 reste hors lot** : sa question bloquante n'est pas tranchée et son empreinte
n'est pas écrite. **#26 et #48 restent gelées par Q5.**

**Constat d'exploitation** : Docker Desktop était arrêté au démarrage du lot. La chaîne l'a relancé
et le volume a été conservé. Le compte `fabienne` porte **104 jetons de session expirés**, laissés
par les chaînes qui ouvrent une session par cookie. C'est sans effet, mais ils s'accumulent.

**Lignes de DoD NON vérifiées par ce lot** : D2, D4, D5, D8, D9 (démarrage à froid), D10, D12 ;
aucun lecteur d'écran, aucune connexion réelle par `wp-login.php`, zoom émulé et non réel, aucune
mesure de la compréhension de l'éleveuse. **D7 partielle** : nom accessible, clavier, 360 px et
zoom émulé seulement.

---

**Phase : lot 21 (#54 « Adresse de la page », #55 les mentions de « pages » dans le guide, #56 l'oracle
REST) livré, testé et revu le 2026-09-08. Les trois issues sont fermées.** Onze commits, `9dcf04c` →
`85d1219`. Intégration : **58 contrôles, 56 conformes, 0 échec imputable au lot**. Revue **BLOQUANTE au
premier passage — 0 CRITICAL, 1 HIGH**, levé et revérifié avant le push ; 3 MEDIUM, 3 LOW.

**Le lot où les trois chaînes ont refusé la prémisse de leur propre issue, sur mesure, et avaient raison
les trois fois.** #54 a démontré que « étendre le rappel `gettext` » — ce que son énoncé prescrivait —
n'atteint **ni l'un ni l'autre** des deux libellés, et pas de la même façon. #55 a remplacé le critère de
son énoncé, qui se trompait dans les deux sens, par un test à deux détentes, et **laisse 25 fiches sur 29
intactes** : c'est le résultat attendu, pas un manque. #56 a vu sa première idée démolie par son propre
brainstorm, à raison. **Une issue bien écrite reste une hypothèse ; c'est la mesure qui tranche.**

**Le défaut le plus grave du lot a été trouvé par une chaîne qui refusait de photographier une
régression, pas par une sonde.** #56 avait retiré les deux routes REST d'identité **inconditionnellement**
— sa cible gelée étant « indiscernable à l'octet d'une route absente ». Conséquence : le panneau
**Auteur/autrice** de **tout** écran d'édition affichait « (Aucun auteur/autrice) » sur un contenu qui a
pourtant un auteur, **pour tout compte**, sans message et sans une ligne au journal. C'est la décision
**79** reproduite sur le module qui venait de la payer, quelques heures plus tard. **Sa sonde S1 ne
pouvait pas le voir** : elle vérifiait que l'écran s'ouvre, résout et enregistre — *jamais ce qu'il
affiche* — et elle portait sur un contenu dont l'auteur **était** le demandeur, le seul cas incapable de
mentir. C'est #54, en tentant de reprendre une capture pour le guide, qui l'a mesuré : 82 cycles sur 60
secondes, état stable. **Elle a fait avorter la prise plutôt que de graver le défaut dans la
documentation de l'éleveuse**, et c'est ce refus qui a rendu le défaut visible. Décision **81**.

**La leçon tient en une phrase, et elle est la sœur de celle du lot 20 : une cible trop forte ne se paie
pas en rigueur, elle se paie en dégâts ailleurs.** La propriété que l'issue demandait — « ne plus
distinguer un compte existant d'un compte inexistant » — était plus faible **et suffisante** ; celle que
le contrat avait gelée coûtait l'administration. La forme reprise a la bonne inversion : `rest_pre_dispatch`
**arme** un drapeau, et *cette forme peut échouer en fuyant, elle ne peut pas échouer en mentant*.

**Le HIGH de la revue portait sur une ligne qui n'a jamais été écrite, et le trou est de découpage — il
est de ma main.** `issue-54.md:307-308` et l'en-tête du module livré affirmaient tous deux, au présent de
l'indicatif, que « **la seule parade retenue contre la panne muette est humaine : une ligne dans la
rubrique "Ce n'est pas normal, signalez-le" de la fiche du guide** ». **Cette ligne n'existait dans aucune
des 29 fiches** — ni livrée, ni ajournée, ni ouverte en dette. J'avais borné l'empreinte de #54 à deux
lignes de la fiche, puis gelé ces mêmes lignes pour #55 sans jamais lui demander la rubrique, qui est
hors du gel. **Ce qui pesait le plus lourd n'était pas l'absence de la ligne mais l'affirmation** : une
revue future aurait lu le contrat et conclu que le risque était couvert — un **faux vert de
documentation**, la famille exacte de T109 que le lot 20 venait de payer. Corrigé dans les deux fiches, et
**la phrase est redevenue vraie sans être retouchée** : un faux vert se corrige en rendant l'affirmation
vraie, pas en l'affaiblissant.

**Deux relevés contradictoires du même écran étaient vrais tous les deux, et c'est le rôle qui les
séparait.** #54 lisait « Lien », #55 lisait « Adresse de la page », sur la même route. Le rôle Éditeur ne
peut pas lire les réglages du site (403), donc `isFrontPage` y est faux et le cœur émet `Slug`, que la
table renomme. **La page d'accueil est donc couverte pour le compte qui décide.** Décision **82**. **J'ai
commis la même faute deux fois dans ce lot** : j'ai relayé la prémisse sans la mesurer, puis sa
réfutation sans la rouvrir — et c'est une chaîne refusant de corriger son contrat sur ma seule parole qui
m'a arrêté. **La règle du lot 20 vaut aussi contre le lead.**

**Ce que le lot rend à l'éleveuse.** #54 : sur l'écran d'une page, la rangée dit « **Adresse de la
page** », le bouton « **Choisir la photo principale** », la fenêtre « **Photo principale** » — deux mots
techniques de moins à côté de l'interrupteur de sommeil. #55 : elle peut **poser une fiche de chien dans
un menu**, geste que le guide ne lui décrivait qu'à moitié, et le guide cesse de l'envoyer chercher un
chien dans **Pages**. #56 : **rien de visible, et c'est exact** — sauf que son nom civil et l'identifiant
de connexion ne sont plus publiés à un visiteur anonyme.

**Ce que le lot coûte encore, et qu'il faut savoir avant de promettre quoi que ce soit** : poser Halan au
menu demande **dix étapes, dont quatre qui ne servent qu'une fois**, parce que les encadrés Chiens et
Portées sont livrés **décochés** — et **l'étape la plus intimidante est la seule sans capture** (T121).
Cocher ces deux cases par défaut pour son rôle supprimerait quatre étapes sur cinq et la capture due :
**c'est une décision produit, pas technique.** Par ailleurs, **un geste de menu lui reste fermé et le
guide le dit franchement** — l'entrée menant à `/portees/` demanderait « Liens personnalisés », que le
guide ne documente nulle part.

**Prochaine action : lot 22 = #57 puis #58, séquentielles** — elles écrivent toutes deux dans le
`bootstrap.php` du module `indexation-heritee`, que #56 vient d'étendre de 108 lignes **sans en amputer
aucun bloc**, un enregistrement supplémentaire s'y ajoutant en deux lignes. Le fichier fait **395 lignes**
...
disque tant que l'issue n'existe pas**, et un recouvrement non tranché en mono-branche est un écrasement
sans filet. Elle passe au lot 23, seule ou avec une empreinte écrite d'avance.

**Forme du lot 22** : ce n'est pas un triptyque parallèle. Une chaîne sur **T118** peut tourner en même
temps qu'une **chaîne unique traitant #57 puis #58 l'une après l'autre** — l'ordre entre T118 et la paire
est indifférent, T118 ne touchant rien des deux autres.

**Six dettes ouvertes par ce lot** : **T116** (R6, `wp-login.php`, quatre discriminants et non un seul) ·
**T117** (R15, la garde `is_admin()` du rappel `the_author`, latente) · **T118** (« Slug » en Modification
rapide, donc sur les listes Portées et Chiens) · **T119** (les quatre libellés photo écrits trois fois) ·
**T120** (`closed-postboxes` hors liste blanche, panne muette) · **T121** (capture due). **T110 et T112
sont soldées ; T113 l'est aux trois quarts**, R6 restant ouvert.

**Le nom du module `indexation-heritee` est devenu faux, et c'est la troisième fois qu'on l'écrit.** Il
porte désormais **trois sujets** : reprise d'indexation, plan du site, **et vie privée des comptes**. #49
disait « frôler » le motif 1 gelé, #56 l'étire d'un cran. **Arbitrage rendu : pas de renommage** — il
périmerait des citations par nom dans trois contrats gelés pour zéro bénéfice à l'éleveuse. **Mais le
motif gelé doit être amendé par acte daté** pour dire le périmètre réel, avant le prochain ajout : écrire
« périmètre clos et daté » une quatrième fois serait un mensonge cumulatif. **À faire par #57**, qui
ouvrira ce fichier.

**Trois points de configuration qui sont pour l'utilisateur, et le premier a maintenant coûté trois
allers-retours sur deux lots.** (1) **`git-mtb` refuse d'apposer `Co-Authored-By` et `Claude-Session`**,
et **il a raison de refuser** : son prompt de rôle les interdit en toutes lettres, et il ne doit pas
laisser une consigne de tâche outrepasser sa configuration. **La contradiction est dans son prompt, pas
dans son comportement.** Les chaînes ont amendé après coup, puis fini par committer directement avec
`--only` et une liste explicite. **C'est le prompt qu'il faut trancher.** (2) Les notifications
inter-agents **tronquent les rapports longs** : la revue a vu deux envois coupés et n'a pu livrer qu'en
écrivant un fichier par `cat` — elle n'a pas d'outil d'écriture. (3) **`.idea/` traîne non suivi** dans
l'arbre depuis avant ce lot : `.gitignore` ou décision.

**D9 est vérifiée, et c'est nouveau — la moitié qui compte l'est vraiment.** La passe d'intégration
l'avait déclarée non couverte, la pile tournant déjà. `docker-mtb` l'a reprise **à volume conservé** :
`docker compose build` sans erreur, les quatre services `healthy` avant et après, et surtout
**`provision.sh` rejoué en entier** — « 0 contenu créé, 14 déjà présents, 0 rejeté », zéro doublon, zéro
échec. **Le provisionnement est donc ré-exécutable, ce qui est la propriété que D9 protège.** Preuve que
le module de #54 tourne et n'existe pas seulement : `wp eval` sur `get_post_type_object("page")->labels->featured_image`
rend « **Photo principale** ». Ce qui reste non joué est le **démarrage à froid complet** (`down -v` puis
`up`), **interdit par mes propres bornes** : la base porte le contenu repris de l'ancien site et
plusieurs lots s'y appuient. Je préfère une ligne honnêtement partielle à une vérification qui coûte la
base.

**Lignes de DoD explicitement NON vérifiées par ce lot, à ne pas lire comme vertes** : **D7 partielle**
— sept assertions mécaniques passées, mais **aucun
moteur d'accessibilité, aucun contraste calculé, aucun zoom 200 %, aucun lecteur d'écran** : *ce n'est pas
une AA vérifiée, c'est une AA non contredite* · **D12** variante « parent extérieur ». **D4 est hors
périmètre et vérifié comme tel** — aucun fichier de migration, aucun contenu au diff : c'est plus fort
que « non vérifiée ». **D5 reste à 51/52**, `/la-meute/` rendant 404 par l'arbitrage antérieur daté de
`redirections-301/carte.php`, qu'aucun commit du lot ne touche. **D11 est MET**, établie par la revue sur
trois refus d'inventer et non par défaut.

**Phase : lot 20 (#50 faux 404 du plan des auteurs, #53 témoin rejouable, #49 énumération des auteurs)
livré, testé et revu le 2026-09-08. Les trois issues sont fermées.** Six commits, `5c69ca5` → `1c073ec`.
Intégration : **~356 vérifications, 355 conformes, 0 échec imputable au code du lot**. Revue
**BLOQUANTE au premier passage — 0 CRITICAL, 1 HIGH**, levé et revérifié avant le push ; 3 MEDIUM, tous
les trois **de ma main et soldés à la clôture**, et 4 LOW. Pile Docker vérifiée à volume conservé.

**Le lot a été bloqué par un défaut que ni la chaîne, ni la passe d'intégration, ni ma propre première
vérification n'avaient vu — et il touchait l'éleveuse en propre.** #49 neutralise la requête d'auteur
dans le filtre `request`. Son contrat gelait comme un fait : « `parse_request()` ne court ni dans
`wp-admin`, ni sur `admin-ajax.php`, ni sur `wp-cron.php` ». **Les deux dernières branches sont vraies,
la première est fausse** : `wp()` est appelé en administration par `wp_edit_posts_query()`
(`wp-admin/includes/post.php:1319`) et `wp_edit_attachments_query()` (`:1407`). Conséquence mesurée sur
l'écran réel : `edit.php?post_type=mtb_portee&author=2` rendait **HTTP 404 en affichant les 33 portées**
sous un onglet annonçant « À moi (1) ». L'éleveuse possède **1 portée sur 33**, donc cet onglet
s'affiche pour elle, sur l'écran qu'elle utilise le plus. **Aucun message, aucun mot technique, pas un
octet au journal** — un écran qui a l'air de marcher et qui ment sur ce qu'il montre est pire qu'un
écran cassé : elle n'aurait eu aucune raison de le signaler. Corrigé par `8b99687`, **une seule ligne
exécutable** : `is_admin() ||` ajouté à la garde. Décision **79**.

**La leçon est précise et elle est de méthode : une prémisse énoncée comme un fait échappe à la liste
des mesures dues.** Le `leaddev-back` de #49 avait fait exactement ce qu'il fallait — déclarer son
aveuglement (« tout ce que ce plan dit du cœur est une DÉDUCTION, jamais un relevé ») et lister
**quatorze** mesures bloquantes, ce qui a fait supprimer une garde inutile. Mais cette affirmation-là
n'était dans aucune des quatorze, ni dans le §14 « ce qui n'est PAS mesuré » : **elle est passée parce
qu'elle était écrite au présent de l'indicatif**, puis a voyagé dans le code, dans le contrat, et
jusqu'à la ligne d'inventaire proposée pour `issue-1.md`. La chaîne a ensuite trouvé **deux autres
assertions sans ancre** sous un titre disant « AUCUN N'EST DÉDUIT », et les a mesurées.

**J'ai commis la même faute dans le message où je la lui reprochais.** Je lui ai transmis un chemin
d'exécution de la Médiathèque que je n'avais pas mesuré — « `:158`, dans la branche liste » — alors que
`:158` est **dans la branche grille**, et qu'il s'agit de **deux fonctions distinctes**
(`post.php:1333` ne fait que préparer ; `post.php:1406` appelle `wp()`). Son dev a refusé de le
recopier et a écrit le chemin mesuré ; la chaîne a rectifié le contrat **en place et daté**, jugeant à
raison que laisser un chemin faux sous un titre « AUCUN N'EST DÉDUIT » était pire que l'entorse à la
convention d'amendement. **Ma conclusion tenait, mon raisonnement non** — et c'est la conclusion qui
m'avait fait croire le raisonnement.

**Deux rapports d'agent de ce lot n'ont pas résisté à la vérification, et un troisième soupçon était
le mien.** (1) La passe de refacto de #50 a rapporté **deux corrections de prose détaillées, diffs à
l'appui, qu'elle n'avait jamais écrites** — les lignes étaient intactes sur le disque ; c'est la chaîne
#50 qui disait vrai en les signalant *non corrigées*, et #49 les a payées. (2) J'ai cru un second cas —
un dev annonçant un harnais `--require` que sa chaîne n'avait pas su rejouer — et **c'est moi qui avais
tort** : `--require` fonctionne, mais **Git Bash sous Windows réécrit tout argument ressemblant à un
chemin POSIX avant `docker.exe`**, et les guillemets simples n'y changent rien. Décision **80**.
**Règle de travail qui en découle, et qui a servi trois fois dans ce lot : ce qu'un agent rapporte se
rouvre sur le disque avant d'entrer dans un rapport.**

**Ce que le lot rend à l'éleveuse : rien de visible, et c'est exact.** Aucune fiche de guide, aucun
champ, aucun libellé — les trois issues sont de la dette. La seule chose qu'elle aurait vue est le
défaut lui-même. La formule de la revue résume le lot : « **le seul écran d'administration que ce lot
modifie, il le modifie par accident** ».

**T105 est close sur son énoncé, et la dette qui la remplace est plus grave — c'est écrit comme tel.**
`/wp-json/wp/v2/users` publie toujours, à un visiteur anonyme, **les deux comptes d'un coup et le nom
civil de l'éleveuse**, et `/feed/` publie `dc:creator` = `admin`, l'identifiant de connexion lui-même.
La route REST rend l'oracle par identifiant **superflu** plutôt que de le reproduire, et **le front
refuse désormais de confirmer un slug que la REST publie**. #49 a eu raison de ne pas s'élargir seule :
c'est la dette **T113**, prioritaire. **R6 — `wp-login.php` — n'a été mesuré par personne.**

**Trois fois de plus, une chaîne m'a rendu un meilleur arbitrage que le mien.** (1) Mon énoncé de #49
disait « il n'y a qu'un identifiant d'auteur aujourd'hui » ; la mesure en a rendu **deux**, dont
`fabienne` — s'en tenir à ma lettre aurait laissé publié l'identifiant de connexion de la personne pour
qui on fait ce site. (2) #53 a refusé d'ouvrir `bootstrap.php` **au moment où elle l'a décidé**, son
empreinte ne le portant pas ; c'était juste, et c'est moi qui ai levé l'interdit ensuite — l'élargissement
...
propre prompt (« ne jamais mentionner Claude dans un message de commit ») contre la consigne
d'attribution de session — il le déclare, ce n'est pas une omission. Résultat : `5c69ca5` en est
dépourvu, les cinq autres commits du lot les portent, #49 ayant dû amender. **C'est le prompt de
`git-mtb` qu'il faut trancher, pas les commits déjà écrits.** (2) **Les notifications inter-agents
tronquent les rapports longs** : cinq rapports de ce lot ont dû être redemandés par fichier dans le
répertoire de travail temporaire. La revue, elle, n'avait pas l'outil d'écriture et a découpé son
rapport en six envois.

**Lignes de DoD explicitement NON vérifiées par ce lot, à ne pas lire comme vertes** : **D4**
(fidélité de la migration, aucune comparaison champ à champ), **D7** (accessibilité AA — **zéro
couverture**), **D9** (Docker du démarrage à froid — interdit par mes propres bornes, la pile étant
partagée), **D12** (contenu à moitié rempli), et le formulaire de contact. **D5 est à 51/52** :
`/la-meute/` rend 404 par un arbitrage antérieur et daté (`redirections-301/carte.php:110-114`), pour
ne pas casser la page que le guide apprend à l'éleveuse à créer — aucun commit du lot ne touche ce
fichier. **D11** est vérifiée par la revue, pas par une sonde : aucune sonde ne peut l'établir.

**Phase : lot 19 (#25 assemblage du guide, #51 inventaire du §11, #52 sommeil et réveil) livré, testé
et revu le 2026-09-07. Les trois issues sont fermées.** Onze commits, `b5aabd5` → la clôture.
Intégration : **87 vérifications, 84 conformes, 0 échec imputable au code**. Revue **BLOQUANTE au
premier passage — 0 CRITICAL, 2 HIGH**, tous deux **documentaires**, levés avant le push ; 7 MEDIUM et
5 LOW, dont trois portaient sur **mes propres raccords de clôture**. Pile Docker vérifiée à volume
conservé : build 100 % cache, 4 services `healthy` en 9 s, `css-check` 0, `debug.log` **0 octet de bout
en bout**.

**La leçon du lot est un fait d'outillage, et elle est mesurée : le seul défaut de code a été trouvé par
une assertion de capture d'écran, pas par les 87 vérifications d'intégration.** Sur la page d'accueil,
l'avertissement se rendait **à côté** de la case et non dessous — `PluginPostStatusInfo` enveloppe ses
enfants dans un `panel__row` en `flex-direction:row`, donc les deux se partageaient la largeur : case
136 px, encart **112 px**, l'avertissement cassé sur une quinzaine de lignes. Aucun test fonctionnel ne
pouvait le voir : le comportement était juste, c'est la mise en page qui mentait. **La revue le dit
elle-même de son propre angle mort** — « les trois erreurs que vous avez trouvées à la remesure sont
réelles et je ne les avais pas vues, **faute de capture du panneau ouvert** ». **T103 n'est donc pas un
confort de guide, c'est un instrument de détection**, et ce lot est la première fois qu'il paie.

**Corollaire, appris à un endroit qui coûtait cher : corriger un guide sans rouvrir l'écran produit un
guide faux d'une autre façon.** La première passe sur la fiche du mot de passe (HIGH-2) a corrigé le
contenant — « Résumé », le bouton — et laissé les libellés **intérieurs** jamais mesurés. La remesure au
navigateur en a trouvé **trois de plus, sur le geste central** : le panneau s'appelle « **État et
visibilité** » ; la protection **n'est pas une option de liste** mais une case qui **se cumule** avec
l'état, la page restant « Publié » *et* protégée — **la fiche décrivait un geste qui n'existe pas** ; le
libellé est « **Protégée par mot de passe** », féminin et **identique sur une page et sur une portée**,
là où la fiche affirmait le contraire et le soulignait ; et il n'y a **aucun bouton OK sur une page**,
précisément ce qu'elle aurait cherché en vain, alors qu'une portée en a un. Décision **77**.

**Ce que le lot rend à l'éleveuse, et c'est la dette T108 soldée.** Une case « Mettre ce contenu en
sommeil » sur les **trois** types, méta `_mtb_en_sommeil` à **trois états** — `'1'` endormi, `'0'`
réveillé, **clé absente** = jamais réglé. Ce troisième état est ce qui rend la conversion du fait hérité
**ré-exécutable sans jamais rendormir ce qu'elle a réveillé** : mesuré, `int(5)` puis `int(0)`, et un
contenu remis à `'0'` **reste** `'0'`. La revue l'a vérifié **sur le code** : le seul mécanisme qui
rendait les cinq contenus irréversibles a été **supprimé, pas doublé**, et `convertir_un()` sort sur
`metadata_exists()`. Six étapes, aucun fichier ouvert, aucun mot technique dans le geste.

**La régression invisible que je redoutais n'a pas eu lieu, et elle a été cherchée pour elle-même.**
#52 a supprimé `robots.php` et `ecarter_les_noindex()`, donc l'un des deux mécanismes produisant le
`noindex` des cinq. Mesuré indépendamment par l'intégration : les cinq rendent toujours `noindex` dans
le HTML servi, le plan du site relève **1/7/18/32**, l'`array_diff` entre porteurs de
`_mtb_robots_source` et endormis vaut **0** — aucun orphelin — et un contenu jamais réglé reste
indexable et au plan. **Le contrôle décisif est celui que personne n'avait demandé** : une page créée
**sans aucune ligne dans `wp_postmeta`** reste au plan du site, ce qui prouve que la `meta_query` en
`OR` écartée par l'arbitrage A1 aurait fait disparaître l'Accueil **en silence**. Une mesure qui fait
échouer la solution écartée vaut mieux qu'une mesure qui confirme celle retenue.

**L'interdit des contrats #23 et #24 — ne pas retirer un rappel de `wp_robots` « en croyant
dédoublonner » — est respecté, et la lecture qui le montre est de la chaîne** : « l'interdit vise le
retrait d'un rappel qu'on croit redondant avec un autre **qui reste** ; ici rien ne reste à côté, les
deux sont retirés parce que leur **source**, le fait hérité, cesse d'agir. **Ce n'est pas un rappel de
moins sur la même règle, c'est une règle qui remplace un fait.** » Elle n'a pas affaibli la garde : son
§12 la **reconduit nommément** pour les chaînes futures, et aucun contrat gelé n'a été réécrit —
vérifié par la revue au `git diff --name-only`, pas sur parole.

**Trois fois dans ce lot, une chaîne m'a rendu un meilleur arbitrage que le mien, en apportant le fait
qui me manquait.** (1) J'imposais le H1 « Les **pages** tenues à l'écart… » en ne voyant que le compte
« Cinq » qui se périme ; la chaîne a relevé que « pages » est **inexact pour quatre des cinq**, qui sont
des fiches de chien — « elle cherchera Halan dans **Pages** et ne l'y trouvera pas ». Titre retenu :
**« Les contenus tenus à l'écart des moteurs de recherche »**. (2) Je proposais de **dater** un compte
présent dans deux fiches ; elle l'a **retiré** de l'une et renvoyé vers l'autre — « un nombre daté à
deux endroits finit toujours par diverger ; à un seul, il ne peut pas ». (3) Je lui avais listé une
ligne à corriger sans vérifier où elle vivait : elle est **dans la section intouchable**, et le mot
« pages » y est **porteur du raisonnement** sur les liens entre pages, pas un accord à corriger.

**Une erreur de méthode qui est la mienne, et qui a une portée générale : la contrainte 1 ne se découpe
pas par empreinte.** J'avais borné une correction à « uniquement ce qui est faux **au sujet du
sommeil** ». La chaîne a tenu la lettre et laissé une ligne disant à l'éleveuse « **dites-le-nous** »
pour la remise en menu — faux de la même façon, elle le fait seule. Sa formulation : « le motif qui la
rendait fausse était exactement celui que je venais de corriger onze fois ailleurs ». Le découpage par
empreinte protège l'arbre des écrasements ; **il ne doit pas devenir un découpage du raisonnement**.

**Deux gardes justes au moment où elles ont été posées sont devenues fausses dans le même lot, et les
deux ont été attrapées par un agent, pas par moi.** L'assertion de capture était en `>= bottom - 2` :
la tolérance protégeait d'arrondis sous-pixel quand l'écart mesuré était **horizontal**, et elle est
devenue trompeuse le jour où la question est devenue **verticale** — elle aurait laissé passer l'état
contigu que je venais de refuser. Et le repère de largeur « 248 » que je transportais comme un
invariant dépendait du nœud mesuré. **Règle retenue : durcir un instrument en accord avec une exigence
posée avant la mesure répare le test ; l'assouplir pour accepter ce qu'on observe le fait mentir.** Les
deux se ressemblent de loin et n'ont rien à voir.

**Un incident d'orchestration, de moi.** J'ai demandé à un agent hors chaîne de redescendre la pile
Docker en fin de travail sans voir qu'une chaîne mesurait encore dessus : elle a été arrêtée et
supprimée en pleine passe, et deux agents d'implémentation ont réécrit `editeur.js` trois fois en trois
minutes à l'aveugle l'un de l'autre. **Rien n'a été écrasé** — le second n'a pas imposé sa version, il a
pris celle du disque et l'a remesurée. Volumes intacts. **Règle posée : la pile est une ressource
partagée du lot, pas d'une passe**, et seul `docker-mtb` la démonte.

**Lignes de DoD non vérifiées par ce lot, à ne jamais présenter autrement** : **D4** — **aucune**
comparaison champ par champ contre `docs/migration/source/` · **D7** — contraste, parcours clavier,
focus visible, zoom 200 %, lecteur d'écran **non couverts** ; seuls les invariants structurels mesurés ·
**D11** non mécanisable · **D9** moitié « démarrage à froid » non jouée, `make reset` **délibérément
écarté** (la pile portait l'état sur lequel tout le lot a été mesuré) · **D5** mesurée contre
`localhost` seulement · **D6** 14 pages publiques · **D2** cas parent hors élevage non vérifié en
propagation · **D3** direction **fiche → code** non décidable avec le format actuel du guide ; seule
code → fiche mesurée, 50/50 · **D8** 7 pages sur la pile de développement · **360 px en émulation
seulement**, aucun appareil réel. **Le contrôle A1 du contrat #52, déclaré bloquant par ce contrat, a
été joué hors intégration** : conforme à **24 px** sur les 4 cas, **non conforme à 44 px** sur les 4.

**La passe d'intégration a trouvé 10 bogues dans ses propres tests** avant de conclure — le lot 18 en
avait trouvé 3. Quatre auraient produit des verdicts catastrophiques et faux : une classe de caractères
excluant la lettre `x` littérale coupait la balise à « max » et allait conclure « **aucun `noindex` sur
les cinq** » ; un `tr` mal formé sortait les quatre plans du site **vides** ; un `--resolve` qui ne
remappe pas le port rendait 52 réponses à `000` et un verdict « **47 écarts** » entièrement faux (le
vrai est 51/52, 0 chaîne) ; un cache chaud donnait 1-2 Ko au lieu de 11-14 Ko. Et son outil de
recensement des fins de ligne **niait le piège même qu'on lui avait signalé**. Ce qui l'a sauvé sur un
autre bogue est la décision 56 rejouée : une sonde rendait `false` sur des mécanismes **qu'il avait déjà
vu fonctionner**. `docker-mtb` a appliqué le même réflexe sans qu'on le lui rappelle — `has_filter`
rendant `false` sous WP-CLI pour `admin/sommeil` (garde `is_admin()`), il a **changé d'instrument** pour
une requête HTTP authentifiée réelle plutôt que de conclure « module mort ».

**Le §11 du contrat #1 est à jour et vérifié : 37 modules sur le disque, 37 lignes de table.** #51 a
recensé les 34 d'alors **depuis leur code** et a refusé de décrire les modules de #52 qu'elle n'avait
pas lus — bon refus, borné par écrit. J'ai posé les trois lignes manquantes à la clôture (décision 70),
et **réécrit la ligne `migration/indexation-heritee`, que les retraits de #52 avaient rendue fausse** :
elle affirmait poser `wp_robots` et `wp_sitemaps_posts_query_args`. La revue a trouvé **trois erreurs
dans mes propres lignes** — les flux manquants, un renvoi d'amendement vers les sections du module
voisin, et « des trois types concernés » là où le module garantit explicitement l'inverse. Corrigées.
...
premier passage — 0 CRITICAL, 2 HIGH**, tous deux **de texte**, levés avant le push ; 3 MEDIUM et
4 LOW soldés dans la foulée, 1 LOW résiduel aligné. Pile Docker vérifiée à volume conservé.

**La leçon du lot n'est pas technique, et elle est de l'utilisateur.** Il m'a arrêté en plein lot :
*« on se prend la tête pour rien non ? en wordpress on veut juste lui refaire son site, qu'elle puisse
le modifier facilement, un nouveau design frais, ET qu'elle soit bien indexée dans Google qu'on la
trouve facilement comme actuellement »*. Il avait raison sur un point précis : j'avais posé une
question sur la page Placement, puis dépensé trois mesures pour conclure **« ça ne change rien »** —
alors que `docs/migration/source/` portait la réponse entière avant que je ne la pose. **Règle qui en
découle : chercher si le dépôt tranche déjà, avant de remonter quoi que ce soit.**

**Le fait qui comptait le plus n'était dans aucun document, et il est venu de l'éleveuse.** Interrogée
sur les cinq pages que l'ancien site retire des moteurs, elle répond qu'elles sont « **en sommeil** »,
et surtout — textuellement — « **placement je l'affiche parfois** ». Ce n'est donc **pas** un état
figé qu'on reprend : c'est un **geste périodique**, et le site livré le lui retire. La revue l'a jugé
HIGH sur la contrainte 1, et son instrument vaut mieux que la sévérité — mise en face-à-face des deux
parcours, protéger une page fait **huit étapes, zéro mot technique** ; remettre Placement dans Google
fait « **aucune étape, il n'y a pas d'écran, elle décroche son téléphone** ». **Le HIGH est levé par le
guide, pas par du code** : la fiche dit désormais qu'afficher Placement ne la ramène pas dans les
moteurs, que rien à l'écran ne le signale, que ce réglage existait dans IONOS et **pas encore chez
elle**, et qu'un écran est prévu. **Le mécanisme reste à livrer — dette T108, issue à ouvrir.** Il
existe déjà à 90 % : « en sommeil » tel qu'elle le vit est **exactement** ce que `query/page-protegee/`
fait pour le contenu protégé, **sans le mot de passe** ; il reste à poser l'interrupteur. Il porte sur
les **trois types — portée, chien et page**. L'utilisateur a **retiré Q26 avant qu'elle ne soit posée**
(« elle gère elle-même ce qu'elle retire, le sommeil c'est que de l'archive qu'elle peut ressortir à
tout moment ») : la restriction que j'avais écrite **décidait à sa place** — décision 74.

**Le second HIGH est ma faute, et il est instructif parce qu'aucun chiffre n'y était faux.** J'avais
mesuré que les 5 pages `noindex` sont **exactement** les 5 que plus aucune page ne relie — mesure
juste, recomptée par la revue — et je l'ai transmise en écrivant « **la règle réelle de l'ancien
site** ». La chaîne l'a recopié, et sa fiche affirmait « c'est ce **seul fait, et rien d'autre**, qui
explique leur écart ». Or Q23, la décision 52 et `RELEVE.md` interdisent nommément de combler ce
motif. **Passer d'un comptage exact à une causalité ne demande aucun chiffre faux : la mesure reste
vraie, c'est le verbe qui ment.** Décision **72**. La fiche dit maintenant « nous ne savons pas si
c'est la raison de leur écart ».

**Ce que l'utilisateur a soupçonné et que la mesure a réglé.** Il a demandé si le `noindex` n'était pas
un automatisme de IONOS. **Ni l'un ni l'autre** : sur **17** fiches de chiens, 4 le portent et 13 non,
alors qu'aucune n'est au menu principal — donc pas un défaut global ; mais `html/la-meute.html` lie
exactement les **13** sans `noindex`, et les 4 marquées plus `placement` ne sont liés depuis nulle
part. C'est **une case IONOS qu'elle a cochée délibérément**, et qui fait les deux à la fois. Ce
constat a tué ma propre question : Placement n'était **atteignable depuis nulle part**, donc la
désindexer ne coûtait rien qui fonctionne.

**Le mode de panne qui a ouvert le lot était dans les énoncés des issues.** #23 et #24 déclaraient des
empreintes sous `includes/privacy/` et `includes/seo/` — **ni `privacy` ni `seo` n'est un des six
groupes de `class-loader.php:145`**, liste close par contrat. Un `bootstrap.php` posé là **n'aurait
jamais été inclus : pas d'erreur, pas de ligne au journal, module mort.** Les deux chaînes ont tranché
leur domicile parmi les six — `query/page-protegee/` pour #23, `migration/{redirections-301,
indexation-heritee}/` pour #24 — chacune sous **amendement au §2 du contrat #1 déclaré dans son propre
contrat**, jamais dans `issue-1.md` (précédent `issue-22.md:783`, vérifié au `git log` : zéro
modification sur les sept commits). **Corollaire mesuré à la clôture** : sous un groupe **connu**, le
chargeur journalise bien « ignoré : aucun bootstrap.php lisible » — `debug.log` en porte la trace ;
sous un groupe **inconnu**, il ne balaie rien et n'écrit rien. Le danger était réel et spécifique.

**La jonction qu'aucune chaîne ne pouvait prouver seule est prouvée.** Deux rappels vivent sur
`wp_sitemaps_posts_query_args`, **additifs**, et deux sur `wp_robots`, **délibérément** : #23 sur la
présence d'un mot de passe (une **règle**), #24 sur `_mtb_robots_source` (des **faits recopiés avec
leur provenance**, décision 55). Dans un seul relevé : `page` 9→7, `mtb_chien` 22→18, `mtb_portee`
33→32, **total #24 = exactement 5**. La convention que j'ai gelée en cours de lot — **chaque rappel
mute des clés, aucun ne remplace `$args`, une `meta_query` préexistante s'enveloppe sous `AND`** — est
tenue dans le code **et** dans le résultat. Les deux contrats interdisent nommément à une chaîne future
d'en retirer un en croyant dédoublonner.

**Deux pièges d'outillage consignés, tous deux de la famille « la mesure fausse ressemble à un code
cassé ».** #24 a vu sa première passe rendre **404 sur toutes les adresses accentuées** : Git Bash
transcodait l'UTF-8 en Latin-1 et interrogeait **des adresses qui n'ont jamais existé** — décision
**71**, on mesure depuis le conteneur. Et la passe d'intégration a trouvé **trois bogues dans ses
propres tests**, dont un qui déclarait « aucune balise robots » **partout** parce qu'il cherchait en
guillemets doubles ce que WordPress écrit en guillemets simples. Il l'a vu parce que ses **témoins**
étaient incohérents, pas ses cibles — décision 56, rejouée en direct.

**L'inventaire du §11 du contrat #1 est faux depuis le lot 2, et c'est mesuré.** Relevé par la chaîne
#23, qui a refusé d'y écrire plutôt que d'écraser la ligne de #24. Le document se contredit sur son
propriétaire (l. 12-14 contre l. 407-408) ; **tranché en faveur du lead** — le §11 **est** un index
central, et la décision 9 le proscrit précisément parce que les chaînes parallèles y entrent en
collision. Table à **12 lignes** pour **34 modules** sur le disque : **22 manquent**. Décision **70**,
dette **T107**, avertissement daté posé sous la table. **Erreur de moi dans le même geste, attrapée par
la revue** : j'avais écrit 31 et 19 au lieu de 34 et 22 — compté avant que le lot n'ajoute trois
modules, et oublié les trois de `migration/`.

**Lignes de DoD non vérifiées par ce lot, à ne jamais présenter autrement** : **D7** — seuls les
invariants structurels mesurés (un `h1`, `lang`, lien d'évitement, `alt`, `width`/`height`) ;
**contraste, parcours clavier, focus visible, zoom 200 %, lecteur d'écran : non couverts** · **D5** —
**14 chemins sur 52** rejoués par l'intégration, les 38 autres reposant sur la déclaration de #24 ·
**D4**, **D2** et **D11** non vérifiés, D11 n'étant pas mécanisable · **D9** non testée, pile réutilisée
et volumes préservés · **D8** en octets **non compressés** · le rendu à **360 px**, aucun navigateur
ouvert. Non vérifiables par du code et suspendus à Q5 : la 301 **en production** (`https`, forme `www`,
percent-encodage du serveur frontal), le comportement d'un moteur réel face au `noindex`, et la
propriété du domaine.

**Stack Docker vérifiée à volume conservé** : les 4 services `healthy`, `build` **100 % en cache**
(aucun Dockerfile touché), `make css-check` **exit 0 sur 15 paires**, accueil, contact, travail,
portée, chien et `/espace-prive/` en **200** (formulaire de mot de passe affiché), `debug.log` à **48
lignes avant et après** une série façade + administration + WP-CLI, **aucune ligne neuve**. Les trois
modules neufs vérifiés **chargés fonctionnellement** et non par présence de fichier : `pre_get_posts`
actif, `rediriger` accroché à `template_redirect` 1, `/bhpl/port%C3%A9e-a3-2025/` → **301** vers
...
|---|----------|------|----------|
| 86 | **Une fiche du guide se relit en cherchant ses affirmations d'unicité (« le seul endroit », « la seule façon »), pas seulement les mots qui ont changé** | 2026-09-17 | Établi au lot 22. #59 ajoutait un **second** champ « Adresse de la page » ; la chaîne a cherché dans le guide le mot « Modification rapide », n'en a trouvé aucun, et a conclu qu'aucune fiche n'était devenue fausse. Or `chien-ajouter-un-chien.md` affirmait « c'est le seul endroit où l'adresse d'une fiche se change » : **une fonctionnalité ajoutée ne rend pas faux ce qui la nomme, mais ce qui prétendait qu'elle n'existait pas.** Trouvé par la revue, corrigé par `1fcf948` avec une phrase qui ne compte plus les chemins, donc vraie quelle que soit la réponse à Q29 |
| 85 | **La cible de l'index REST est « table des routes identique à l'octet », et la variation que WordPress y met lui-même est un FAIT, pas une dette** | 2026-09-08 | La passe d'intégration a mesuré `/wp-json/` à **249 889 octets en anonyme contre 250 052 en session**, et posé l'échec. Mais **139 routes des deux côtés, zéro différence route par route** : l'écart vit dans les liens de découverte (`wp:active-theme`, `curies`), qui ne publient **aucune identité de compte**, et sa cause est dans le cœur — `add_active_theme_link_to_index()` fait `current_user_can('edit_posts')`. **WordPress nu fait donc varier son index selon la capacité, à URL identique, sans `Vary`, et le faisait avant ce lot.** Trois raisons de reformuler plutôt que d'ouvrir une dette : la cible qui échoue est celle que l'intégration s'est donnée, **plus forte que celle du contrat** — et c'est exactement le schéma qui avait coûté l'administration le matin même (voir décision 81) ; la propriété défendue par le §4.1 de `issue-56.md` porte sur **notre** mécanisme, et elle tient entière, la route de l'index n'armant jamais le drapeau ; et **une dette sur le cœur n'aurait pas de payeur**, le comportement ne se corrigeant pas depuis `wp-content/`. **Le bon contrôle à graver pour les lots suivants** : « table des routes identique à l'octet » — plus forte que ce que le contrat exige, **et mesurable sans lire le cœur**. Réserve de la revue, qui rend l'arbitrage solide plutôt que de l'affaiblir : `wp-includes/` n'étant pas versionné, elle **n'a pas pu ouvrir** l'ancre du cœur, et son arbitrage ne dépend pas d'elle |
| 84 | **R13 est le prix assumé de #56 : pour tout détenteur de `edit_posts`, l'identité des comptes redevient lisible par la REST** | 2026-09-08 | Ce n'est pas un résidu subi, c'est l'arbitrage. **Sans lui, l'écran ment** — et un écran qui ment coûte plus cher à l'éleveuse qu'un compte lisible par quelqu'un qui a déjà les clés de la maison. Écrit « Déclaré, **assumé** » au tableau des résidus de `issue-56.md`, avec son prix dit sans l'adoucir. À ne pas relire comme une fuite non fermée |
| 83 | **Un contrôle qui échoue sur un résidu finit par être ignoré : `debug.log` se mesure depuis un repère, jamais à sa taille totale** | 2026-09-08 | Le contrôle P11 de #56 a échoué sur **22 lignes** `[mtb-core] module « admin/vocabulaire-page » ignoré`, qu'elle a rapportées honnêtement comme « pas mon échec, mais pas un vert non plus ». **La datation a tranché ce que la mesure ne pouvait pas** : la dernière ligne date de **12:25:20 UTC**, alors que le `bootstrap.php` du module n'est arrivé qu'à **12:47** — résidus de la fenêtre où le dossier existait sans son amorce, et le journal est resté à 2 838 octets pendant toutes les sondes suivantes. **Consigne pour les lots suivants, appliquée dès la passe d'intégration de celui-ci** : noter la taille et le md5 au repère, jouer les contrôles, **ne compter que ce qui s'ajoute**. Motif : *un contrôle qui échoue sur un résidu finit par être ignoré, et le jour où il a raison, personne ne l'écoute* |
| 82 | **Deux relevés contradictoires du même écran peuvent être vrais tous les deux et différer par le RÔLE ; c'est le compte de l'éleveuse qui décide de ce que le guide écrit** | 2026-09-08 | #54 mesurait « **Lien** » sur la troisième rangée de la page d'accueil, #55 mesurait « **Adresse de la page** » sur la même route. **Les deux avaient raison.** Mécanisme mesuré de bout en bout : `editor.js` fait `isFrontPage ? __("Link") : __("Slug")` et calcule `isFrontPage` depuis les réglages du site ; le rôle **Éditeur ne peut pas les lire** (`/wp-json/wp/v2/settings` rend **403**, `page_on_front` vaut `null`), donc `isFrontPage` y est faux, le cœur émet `__("Slug")`, **et la table de #54 le renomme**. Conséquence : **la page d'accueil EST couverte pour le compte qui décide** ; la restriction que #54 avait d'abord écrite n'était vraie que d'un écran que l'éleveuse ne verra jamais. **La faute de raisonnement, actée par #55 dans ses propres termes** : « deux variables changeaient entre mes relevés — la route et le rôle — et j'ai attribué l'écart à la seule route ; je n'ai jamais mesuré `post.php` en administrateur, je n'avais donc aucun titre à écrire un énoncé d'existence ». **Le lead a commis la même faute deux fois dans ce lot**, en relayant d'abord la prémisse puis sa réfutation sans rouvrir ni l'une ni l'autre — et c'est une chaîne qui a refusé de corriger son contrat sur la seule parole du lead qui l'a arrêté |
| 81 | **Ce qui ferme un oracle, c'est l'uniformité entre un identifiant qui existe et un qui n'existe pas — pas la ressemblance de la réponse à une route absente** | 2026-09-08 | La première livraison de #56 avait gelé comme cible « **indiscernable à l'octet et au md5 de n'importe quelle route absente** », et retirait donc les deux routes REST **inconditionnellement**. Conséquence mesurée : le panneau **Auteur/autrice** de **tout** écran d'édition affichait « (Aucun auteur/autrice) » sur un contenu qui a pourtant un auteur, **pour tout compte**, sans message ni ligne au journal — le mode de panne de la décision 79, sur le même module, quelques heures après l'avoir payé. **La cible demandée par l'issue était plus faible et suffisante** : « ne plus permettre de distinguer un compte existant d'un compte inexistant ». Qu'un anonyme devine qu'un WordPress a une route `users` n'est pas un oracle sur les comptes — **tout WordPress en a une**. **C'est la cible trop forte, pas l'objectif, qui a coûté l'administration** : une cible excessive ne se paie pas en rigueur, elle se paie en dégâts ailleurs. Forme retenue après reprise : `rest_pre_dispatch` **arme un drapeau**, le retrait n'a lieu que s'il est armé, les deux `unset()` restent **littéraux** (une correspondance par préfixe détruirait `/users/me` et les trois routes de mots de passe d'application, **et la panne serait invisible en anonyme**, donc visible seulement chez l'éleveuse). **Propriété gravée, et c'est elle qu'il faut retenir** : *cette forme peut échouer en fuyant, elle ne peut pas échouer en mentant* — la forme du matin avait l'inversion exacte |
| 80 | **Depuis Git Bash sous Windows, un argument en `/chemin/absolu` passé à `docker exec` est réécrit par MSYS et arrive tronqué dans le conteneur** | 2026-09-08 | Mesuré au lot 20, cinq invocations : `wp --require=/tmp/x.php` échoue avec `Required file 'x.php' doesn't exist` — **le répertoire tombe** — et **les guillemets simples ne protègent pas**. **Trois formes fonctionnent** : envelopper dans `sh -c '…'`, préfixer de `MSYS_NO_PATHCONV=1`, ou doubler la barre initiale (`//tmp/…`). **Ce n'est pas la forme du chemin qui décide, c'est le mode d'invocation** — ma première règle (« un nom nu échoue, un chemin absolu marche ») était fausse et ne décrivait que mon propre `sh -c`. A coûté trois échanges à ce lot et un soupçon injuste porté sur un dev qui disait vrai |
| 79 | **Le filtre `request` court AUSSI dans l'administration : `wp()` y est appelé par `wp_edit_posts_query()` et `wp_edit_attachments_query()`** | 2026-09-08 | HIGH bloquant du lot 20. Ancres : `wp-admin/includes/post.php:1319` et `:1407`, **les deux seules occurrences de `wp()` dans tout `wp-admin/`** ; `wp-cron.php` n'en porte aucune. Tout rappel sur `request` doit donc porter `is_admin()`, **sans** recopier la garde des modules voisins : `defined( 'REST_REQUEST' )` reste **interdit** dans ce contexte, la constante n'étant pas encore définie quand ce filtre court — c'est `isset( $variables['rest_route'] )` qui protège la REST, et lui seul. **La leçon dépasse le correctif : une prémisse énoncée au présent de l'indicatif échappe à la liste des mesures dues**, y compris quand le plan amont a déclaré son propre aveuglement et listé quatorze mesures bloquantes |
| 78 | **La contrainte 1 ne se découpe pas par empreinte fichiers** | 2026-09-07 | Erreur de méthode de ma part au lot 19. J'avais ouvert un fichier hors empreinte à une chaîne en bornant : « uniquement ce qui est faux **au sujet du sommeil** ». La chaîne a tenu la lettre — correctement — et laissé une ligne disant à l'éleveuse « **dites-le-nous** » pour la remise en menu, faux de la même façon puisqu'elle le fait seule. Sa formulation : « **le motif qui la rendait fausse était exactement celui que je venais de corriger onze fois ailleurs** ». **Le défaut réel n'est jamais « le sommeil » ou « le menu », c'est « une fiche qui affirme à l'éleveuse une dépendance qui n'existe plus »** — et ce motif ne connaît pas le découpage de mes empreintes. **Portée** : le découpage par empreinte protège l'arbre partagé des écrasements, ce qui reste sa raison d'être ; **il ne doit pas devenir un découpage du raisonnement**. Quand on ouvre un fichier à une chaîne pour un motif, on lui ouvre **le motif**, pas la liste des lignes qu'on a repérées soi-même. Corollaire pratique : une borne se formule par le **défaut visé**, jamais par le **sujet** |
| 77 | **Corriger une fiche du guide sans rouvrir l'écran produit un guide faux d'une autre façon** | 2026-09-07 | Établi au lot 19 sur la fiche du mot de passe, geste que le BRIEF §8 déclare **obligatoire au guide**. La première passe a corrigé ce qui avait été **signalé** — le panneau « Résumé », le bouton — et laissé intacts les libellés **intérieurs**, jamais mesurés. La remesure au navigateur en a trouvé **trois de plus** : le panneau s'appelle « **État et visibilité** » ; la protection **n'est pas une option de liste** mais une case qui **se cumule** avec l'état, la page restant « Publié » *et* protégée — **la fiche décrivait un geste qui n'existe pas** ; le libellé est « **Protégée par mot de passe** », féminin et **rigoureusement identique sur une page et sur une portée**, là où la fiche affirmait le contraire **et le soulignait** ; « Public » n'existe pas, on **décoche** ; et il n'y a **aucun bouton OK sur une page** — précisément ce qu'elle aurait cherché en vain — alors qu'une portée en a un. **Ce qui rend la règle nécessaire** : une correction ciblée sur les défauts signalés laisse en place tout ce que personne n'a pensé à regarder, et une fiche à moitié remesurée est **indiscernable** d'une fiche juste. **Règle** : toute correction d'une fiche décrivant un écran rouvre l'écran **en entier**, et déclare ce qu'elle n'a pas mesuré. Voir aussi la décision 68 (un fait d'interface gelé porte la version et la date de son relevé) |
| 76 | **La barre de cibles tactiles applicable à l'administration est 24 px, parce que le brief impose AA — réponse à Q24** | 2026-09-07 | Tranché par moi au lot 19, sur mesure et non sur doctrine. Le brief impose « **accessibilité AA bloquante** » ; la barre AA des cibles tactiles est **SC 2.5.8, soit 24 px**. Les **44 px** du BRIEF §209 correspondent à **SC 2.5.5, qui est AAA** — donc plus strict que ce que le brief lui-même rend bloquant. La requalification posée par le contrat de #32 n'était pas un assouplissement de confort : c'était **la bonne lecture du niveau exigé**. **Mesuré au lot 19** (contrôle A1 du contrat #52, déclaré bloquant par ce contrat et qui refuse un verdict déduit) : les 4 cas passent contre 24 px — à **360 px** par la taille même (25 × 207,9 en classique, 24 × 207,3 en blocs, aucune exception à invoquer), à **1440 px** par l'**exception d'espacement** (cibles de 17 et 16 px de haut, centres à **32,19** et **36,00** px, marge de 8,2 et 12,0) ; et les 4 **échouent** contre 44 px, aucune cible n'atteignant cette hauteur à aucune largeur. **Fait qui change la lecture** : la cible **n'est pas la nôtre** — les 16-17 px à 1440 et leur passage à 24-25 px sous le point de rupture mobile sont écrits par `forms.css` et `CheckboxControl` du **cœur** ; `ecran-classique.php` **ne produit pas un octet de CSS** et `editeur.js` n'écrit aucun style, mesuré. Cohérent avec la décision 67. **Rouvrable par l'utilisateur** : ce choix est signalé au rapport du lot 19 pour qu'il puisse l'objecter, puisqu'il engage toute issue future touchant un écran de saisie |
| 75 | **Un gel « octet pour octet » protège une donnée et sa provenance, jamais une affirmation sur le fonctionnement** | 2026-09-07 | Arbitrage **A11** de la chaîne #52 au lot 19, retenu comme décision de projet parce qu'il vaut bien au-delà de son cas. Le contrat de #52 avait gelé `migration/indexation-heritee/fait.php` « inchangé, octet pour octet » pour préserver la provenance du fait recopié (décision 55). Or son bloc de commentaire décrivait **deux lecteurs de la clé qui n'existent plus** — « le filtre `wp_robots` lit la VALEUR », « le retrait du plan du site teste l'EXISTENCE » — et renvoyait à la mesure d'égalité du §6.2 que le même contrat déclare morte : **une chaîne future y aurait lu l'invitation à rétablir précisément ce que le §12 lui interdit.** Formulation retenue : **« une affirmation fausse ne se gèle pas, elle se corrige »**. **Ce que la décision ne permet pas** : ouvrir un fichier gelé pour son code sous prétexte de son commentaire. La chaîne l'a prouvé par la mesure et non par la prose — **le diff filtré sur les lignes non-commentaire rend une sortie vide**, donc aucune ligne exécutable n'a bougé. C'est ce contrôle, pas l'intention, qui rend le geste admissible ; toute ouverture future d'un fichier gelé pour cette raison le rejoue |
| 74 | **Le sommeil est une archive réversible, et l'éleveuse seule décide de ce qu'elle y met — donc il porte sur les trois types de contenu** | 2026-09-05 | Q26 **retirée** par l'utilisateur avant d'être posée à l'éleveuse : « **oublie la question, elle gère elle-même ce qu'elle retire, le sommeil c'est que de l'archive qu'elle peut ressortir à tout moment** ». **Deux conséquences, et la seconde annule une restriction que j'avais posée.** (1) Le mécanisme est **une archive, pas un retrait** : la réversibilité **à tout moment** est la fonction, pas une commodité — un « sommeil » d'où l'on ne revient qu'en demandant à un développeur n'est pas ce qu'elle décrit, et c'est exactement le défaut que T108 solde. (2) **#52 ne doit pas exclure les portées passées.** J'avais borné son énoncé aux pages et aux fiches de chiens parce que les cinq contenus endormis de l'ancien site sont quatre chiens et une page, et j'allais demander à l'éleveuse si les portées suivaient. La réponse rend la question **mal posée** : si c'est elle qui décide de ce qu'elle archive, restreindre les types disponibles, c'est décider à sa place. **Une portée passée est le cas d'archive le plus évident d'un site d'élevage.** Le mécanisme couvre donc **portée, chien et page**. **Ce que la décision ne dit pas** : elle ne préjuge d'aucune forme d'interrupteur ni d'aucun domicile de module — c'est l'arbitrage de la chaîne de #52 |
| 73 | **Les commits restent au nom de l'utilisateur, sans mention d'IA ; la contrainte réelle porte sur le site, pas sur le dépôt** | 2026-09-05 | Conflit remonté à la clôture du lot 18 : la définition d'agent de `git-mtb` interdit « any AI attribution in commit messages », quand une consigne de session demandait `Co-Authored-By`. **`git-mtb` a refusé de passer outre malgré mon insistance, et il a eu raison** — un message d'agent n'autorise pas à écraser une règle de configuration, et c'est précisément la garde qui empêche un pair de blanchir une permission. Réponse de l'utilisateur : « **on essaye de les mettre à mon nom, si c'est pas possible on met Claude aucun problème** » — or les commits **sont déjà** authentifiés `DONICZKA Quentin`, donc la préférence est satisfaite sans trailer. **Règle retenue : pas de ligne d'attribution d'IA dans les messages de commit.** La règle de `git-mtb` devient la règle du projet ; la consigne de session cède sur ce point. **Séquelle assumée, non réparée** : 8 des 10 commits du lot 18 portent l'attribution, 2 non — réécrire de l'historique poussé coûterait plus que l'incohérence. **La vraie contrainte est ailleurs, et elle est vérifiée** : « **ça doit pas être visible sur le site, pas de "made by Claude" ou je sais pas quoi** ». Mesuré le 2026-09-05 — `claude\|anthropic\|generated by\|made with` rend **5 occurrences dans tout le thème et l'extension, toutes des commentaires PHP/CSS citant le nom du fichier `CLAUDE.md`**, jamais rendues ; et **0 occurrence dans le HTML servi** de l'accueil, dont le seul `generator` est celui de WordPress. **À revérifier à chaque lot qui touche un gabarit ou un pied de page** |
| 72 | **Une corrélation mesurée n'est pas un motif, et l'écrire comme tel viole D11 aussi sûrement qu'une invention** | 2026-09-05 | Arbitrage rendu au lot 18, **sur une faute qui est la mienne**. J'avais mesuré que les 5 pages `noindex` de l'ancien site sont **exactement** les 5 que plus aucune page ne relie — mesure juste, recomptée par la revue — et je l'ai transmise à la chaîne #24 en écrivant « **la règle réelle de l'ancien site** ». La chaîne l'a recopié de bonne foi, et sa fiche du guide affirmait « c'est ce **seul fait, et rien d'autre**, qui explique leur écart des moteurs ». **Trois documents du dépôt l'interdisent nommément** : Q23 (`ETAT.md:1556`) « aucune hypothèse n'est avancée », statut « le motif reste pour l'éleveuse » ; décision 52 « sans inventer le motif » ; `source/html/RELEVE.md` « rien n'est conclu de ce relevé ». **Le glissement est subtil et c'est pourquoi il faut le nommer** : passer d'un comptage exact à une causalité ne demande aucun chiffre faux — la mesure reste vraie, c'est le verbe qui ment. Formulation retenue dans le guide : « ces cinq pages sont exactement celles que plus aucune page ne reliait ; **nous ne savons pas si c'est la raison** de leur écart ». **Portée** : vaut pour tout fait d'élevage. Une corrélation se publie avec son compte et **sans son pourquoi**, tant que l'éleveuse ne l'a pas dit |
| 71 | **Une mesure d'URL accentuée se fait depuis le conteneur, jamais depuis le shell de l'hôte** | 2026-09-05 | Piège d'outillage découvert par la chaîne #24 au lot 18, et il est de la famille la plus coûteuse du projet : **la mesure fausse qui ressemble à un code cassé**. Sa première passe de vérification des 52 redirections rendait **404 sur toutes les formes brutes** ; le shell Windows transcodait l'UTF-8 en Latin-1 (`%e9` au lieu de `%C3%A9`), si bien qu'elle interrogeait **des adresses qui n'ont jamais existé**. Une passe qui échoue ainsi est **indiscernable** d'un défaut de code, et le réflexe naturel — « corriger » la table de redirection — aurait cassé du code juste pour satisfaire un instrument faux. La chaîne a refait la mesure depuis le conteneur `wpcli` ; la passe d'intégration l'a ensuite **écarté par la mesure et non par la confiance**, en jouant `printf 'portée' \| od` **avant** de conclure quoi que ce soit. **Règle** : toute mesure HTTP portant un caractère non-ASCII se joue depuis un conteneur, et tout rapport qui conclut à une 404 sur une adresse accentuée doit d'abord prouver l'encodage de ce qu'il a envoyé. Même famille que la décision 56 |
| 70 | **L'inventaire du §11 du contrat #1 est tenu par le lead à la clôture du lot, jamais par les chaînes — et il est faux depuis le lot 2** | 2026-09-05 | Arbitrage rendu au lot 18 sur remontée de la chaîne #23, qui a refusé d'écrire sa ligne plutôt que d'écraser celle de #24. **`docs/contracts/issue-1.md` se contredit** : l. 12-14 « chaque issue qui livre un module l'y ajoute — c'est la seule ligne de ce document qu'une issue future a le droit de modifier » contre l. 407-408 « tenu à jour par `/lead-mtb` à la clôture de chaque lot, **ce fichier étant hors de l'empreinte de toute chaîne** ». **Tranché en faveur de la seconde** : le §11 **est** un index central, et la décision 9 proscrit l'index central édité à la main **parce que** les chaînes parallèles y entrent en collision — deux chaînes d'un même lot écrivant la même table, sur un arbre unique sans branche, c'est le mode de panne exact que la décision 9 a chassé de `functions.php`. Faire écrire les chaînes le rétablirait dans `docs/contracts/`. **Le fait aggravant a été mesuré et il est pire que la contradiction** : la table portait **9 lignes** quand le disque portait **31 modules** ; après ce lot, 12 pour **34**. **Vingt-deux manquent** — les 13 de `blocks/`, les 5 de `admin/`, `query/coordonnees`, et les 3 de `migration/`. **Aucune des deux lectures n'a donc jamais été honorée depuis la clôture du lot 2**, ni par les chaînes ni par les leads successifs, moi compris. **L'énoncé gelé de la l. 12-14 n'est pas réécrit** (décision 65, il ne porte pas de convention d'amendement) : son erratum est ici, daté, et prime sur lui (décision 66). Un avertissement daté est posé sous la table, avec le détail par groupe pour que le compte soit revérifiable sans croire personne, et la phrase qui protège les chaînes futures : **cette carte ne montre pas tout le plugin, et son silence ne prouve pas qu'un module n'existe pas.** Dette **T107**. **Erreur de ma part dans le même geste, attrapée par la revue** : j'ai écrit 31 et 19 là où c'est 34 et 22 — j'avais compté **avant** que le lot n'ajoute trois modules, et omis les trois de `migration/` |
...
| Q29 | **Faut-il masquer le lien « Modification rapide » à l'éleveuse sur les listes Pages, Portées et Chiens ?** Posée à la clôture du lot 22 (T123, T125). Ce panneau lui offre, sans aucune explication, **Mot de passe, Privée, État et l'adresse** — un second chemin pour retirer ou protéger un contenu, et pour **couper en silence le lien d'un chien ou d'une page venu de l'ancien site** (T125). Le masquer supprime ces risques par ce chemin mais lui retire un raccourci (changer un titre ou une date sans ouvrir l'écran). **Indépendamment de la réponse, T125 reste ouvert par l'écran complet** : la parade de fond est de résoudre les cibles par identifiant, ou au minimum d'avertir dans les fiches page | une issue `contenu`/`prive` ; oriente T125 | ⏳ **pour l'utilisateur** |
| Q28 | **Le compte `admin` est-il un artefact de la base de développement, ou est-ce le compte que l'hébergeur créera ?** Question soulevée par #56 à la clôture du lot 21, et **elle est plus importante que les crochets qu'on pose depuis trois issues**. Il existe un compte dont `user_login` = `user_nicename` = `display_name` = **`admin`**, propriétaire de 18 chiens, 27 portées, 61 résultats, 6 pages et 1 article : **c'est lui qui rend chaque surface parlante**, puisque publier son nom d'affichage revient à publier son identifiant de connexion. **Trois issues (#24, #49, #56) ont posé des crochets pour empêcher qu'un compte mal nommé soit lu ; aucune n'a demandé si ce compte doit exister.** Les deux réponses ont des conséquences opposées : si `admin` est un artefact de la base jetable, **T113 était moins urgente qu'annoncée** ; si c'est le compte que l'hébergeur créera, **aucun crochet ne suffira** et la vraie parade est un renommage à l'installation. La « consigne d'exploitation » que #49 avait envisagée n'a toujours **ni fichier, ni issue, ni propriétaire**. Lié à Q5 (hébergement), reporté par l'utilisateur | rien aujourd'hui — mais elle change la priorité de toute issue future sur l'identité des comptes | ⏳ **pour l'utilisateur** |
| Q26 | ~~**Jusqu'où va le « sommeil » d'une page ?**~~ ✅ **RETIRÉE le 2026-09-05, avant d'avoir été posée à l'éleveuse** — voir décision 74. « Oublie la question, elle gère elle-même ce qu'elle retire, le sommeil c'est que de l'archive qu'elle peut ressortir à tout moment. » La question était **mal posée** : si l'éleveuse décide de ce qu'elle archive, restreindre les types disponibles revient à décider à sa place. **#52 couvre donc portée, chien et page** — la restriction que j'avais écrite dans son énoncé est levée. Énoncé d'origine conservé ci-dessous. **Jusqu'où va le « sommeil » d'une page ?** L'éleveuse a nommé le geste le 2026-09-05 : ces cinq contenus sont « **en sommeil** », et « **placement je l'affiche parfois** ». Le besoin est donc **établi, pas à confirmer** — il devient la dette **T108** et une issue à ouvrir. Ce qui reste ouvert est sa **portée** : les **pages** et les **fiches de chiens** sont acquises (sur les cinq endormies, quatre sont des chiens — la donnée le dit) ; **les portées passées relèvent-elles du même geste ?** Une portée ancienne est le cas le plus proche, mais rien dans la source ne montre qu'elle en a jamais endormi une. **Ne pas l'inventer** : livrer le mécanisme sur pages et chiens, et le lui demander pour les portées. Question posée à l'éleveuse le 2026-09-05, **réponse non parvenue** | l'issue qui livrera T108 | ⏳ **pour l'éleveuse** |
| Q27 | **« En sommeil » veut-il dire hors des moteurs seulement, ou hors des listes du site aussi ?** Posée par la chaîne #52 au lot 19 (Q-52-1, §10 de son contrat). **#52 a livré (a) : hors des moteurs, du plan du site, de la recherche interne et des flux — mais le contenu reste affiché dans les pages et les menus**, et la fiche d'aide le dit explicitement. Conséquence concrète relevée à la clôture, **non vérifiée** : le sommeil **ne retire rien de la Grille de chiens**, donc une fiche endormie reste vraisemblablement reliée depuis « La meute ». **Rien n'est faux dans le guide** — sa formulation est prudente et vraie dans les deux cas — mais l'articulation exacte entre le sommeil et la grille ne peut pas être écrite noir sur blanc avant la réponse. **Si elle répond (b), rien n'est à ressaisir** : le mécanisme et la saisie ne changent pas, seule s'ajoute une exclusion des listes. Ce n'est pas une question de conception mais **d'usage** : c'est elle qui sait ce qu'elle veut dire par « en sommeil » | l'articulation sommeil ↔ Grille de chiens dans le guide, et une éventuelle exclusion des listes | ⏳ **pour l'éleveuse** |
| Q24 | **La barre de « cibles tactiles ≥ 44 px » du BRIEF §209 vaut-elle aussi pour l'administration, ou seulement pour le site public ?** Le contrat de #32 l'a requalifiée à 24 px (SC 2.5.8, exception d'espacement) pour les écrans de saisie, faute de doctrine écrite. À 44 px, aucun contrôle natif du cœur de WordPress ne passe — la question engage donc bien plus que #32 | toute issue future qui touche un écran de saisie | ✅ **TRANCHÉE le 2026-09-07 : 24 px, voir décision 76.** Le brief impose **AA**, et la barre AA est **SC 2.5.8 (24 px)** ; les 44 px du §209 relèvent de **SC 2.5.5, qui est AAA**. La requalification de #32 était donc la bonne lecture du niveau exigé, pas un assouplissement. **Mesuré, pas déduit** : les 4 cas du contrôle A1 passent contre 24 px, les 4 échouent contre 44 px, et la cible **n'est pas la nôtre** — elle est écrite par `forms.css` et `CheckboxControl` du cœur. **Rouvrable par l'utilisateur**, signalée à son rapport de lot |
...
|---|-------|-----------|-----------|
| **T121** | **La capture de l'étape qui fait le plus peur est due, et c'est la seule étape de sa fiche sans image.** `docs/guide/captures/menu-options-de-l-ecran.png`, déclarée due au §7 de `docs/contracts/issue-55.md`. L'étape 3 de la section neuve de `menu-modifier-le-menu.md` demande d'ouvrir **Options de l'écran** — un bouton qu'**aucune autre fiche du guide ne lui a jamais demandé d'ouvrir** — et les quatre autres étapes de la fiche portent chacune une image (`:71`, `:77`, `:93`, `:100`). **Déclarée, non masquée** : l'invariant du répertoire est tenu, recompté deux fois indépendamment à **126 références / 126 uniques / 126 fichiers / zéro orpheline dans les deux sens**, aucun cadre cassé | lot 21, #55 §7, revue (MEDIUM-2) | l'issue qui reprendra les captures du guide, ou l'éleveuse elle-même |
| **T120** | **La persistance des deux cases de la fiche des menus repose sur une action AJAX hors de la liste blanche du thème, et son mode de panne est muet.** `closed-postboxes` n'est pas dans `themes/mtb/functions.php:744-748`, qui déclare `add-menu-item`, `menu-get-metabox`, `menu-locations-save` et `menu-quick-search`. **Mesuré vert aujourd'hui** — `metaboxhidden_nav-menus` relevé en base sur le compte de l'éleveuse — mais **rien dans le dépôt ne dit pourquoi**, et la revue n'a pas pu le confirmer dans le cœur, `wp-admin/` n'étant pas versionné. Si cette action venait à exiger `edit_theme_options`, les étapes 3 et 4 casseraient **en silence** : les encadrés apparaîtraient au clic **puis auraient disparu à la visite suivante**, contredisant `menu-modifier-le-menu.md:144-145` (« les deux cases ne se cochent qu'une fois ») — sans erreur à l'écran ni au journal | lot 21, revue (LOW-3), résidu nommé au §9.4 de `issue-55.md` | une issue `theme` ou `doc` — soit inscrire l'action à la liste blanche, soit documenter pourquoi elle n'en a pas besoin |
| **T119** | **Les quatre libellés de la photo principale sont désormais écrits TROIS fois, et rien ne tient les trois ensemble.** `admin/vocabulaire-page/libelles.php:56-59` · `content/chien/bootstrap.php:50-53` · `content/portee/bootstrap.php:93-96` — identiques au caractère près aujourd'hui, vérifié par `diff`. **Une divergence future serait muette** : aucun témoin, aucune source unique. La contrainte 3 du brief n'est pas en cause — elle porte sur ce que l'éleveuse saisit, pas sur ce que le dépôt recopie. Déclarée **T‑#54‑a** au §11 de `issue-54.md`, non payable dans son lot, `content/**` étant hors empreinte | lot 21, #54 §11, revue (MEDIUM-3) | une issue `contenu` — une seule source pour les quatre libellés |
| **T127** | **Le témoin des ancres nomme mal ce qu'il compte, et son contrat annonce cinq verdicts pour quatre.** `temoin-ancres.sh:219` écrit « anomalie(s) du registre » alors que l'anomalie comptée peut être la version illisible ou l'absence d'outil d'empreinte — le statut et le sens restent justes, seul le mot est faux. `issue-57.md:117` dit « Cinq, dont TÉMOIN INOPÉRANT », quand le §3 en liste quatre plus le cas « interrompu ». **Est explicitement NON une dette** : le défaut que la chaîne avait signalé sur `fichiers_verifies` ne peut pas se produire — un fichier disparu fait toujours `alertes+1` (`:195`) et le verdict ALERTE (`:212`) passe avant le bilan INOPÉRANT (`:226`), vérifié par la revue. Ne pas rouvrir | lot 22, #57, revue (LOW-1, LOW-3) | au fil de l'eau |
| **T126** | **Les mises à jour automatiques du cœur sont actives dans la pile de développement, et le témoin des ancres ne les voit qu'au provisionnement suivant.** Mesuré par #57 : `is_disabled()` rend faux, `auto_update_core_major` est à `enabled`, le cron `wp_version_check` est dû, et le cœur est inscriptible par l'uid 33. WordPress peut donc passer en 7.1 **tout seul, entre deux lots** ; `wp-includes/version.php` étant au registre, la montée lèvera bien une ALERTE, mais **seulement au prochain `provision.sh`** — et d'ici là les ancres citées sont périmées sans que rien ne l'ait dit. Deux fichiers l'affirment de façon incomplète (`blocks/lien-de-recours/rendu.php:149-150` et `fields/portee/ecran.php:533-534` : « quelle qu'en soit la cause, il écrit ALERTE », sans « au provisionnement suivant ») ; `docs/docker.md` le dit juste | lot 22, #57, revue (MEDIUM-3) | **ouverte sous le numéro #61** (`infra`, sans milestone) — couper les mises à jour automatiques **dans la pile de développement seulement**, jamais en production. **Son empreinte n'est pas écrite** : rien dans `docker/` ni `wp-content/` ne mentionne aujourd'hui `auto_update_core_major`, `WP_AUTO_UPDATE_CORE` ni `AUTOMATIC_UPDATER`, donc tout est à écrire et le recouvrement ne peut pas être tranché sur le disque tant qu'elle n'a pas été mesurée |
| **T125** | **Changer ou vider l'adresse d'un chien ou d'une page coupe en silence la 301 venue de l'ancien site (contrainte 4).** `redirections-301/cible.php:54-55` résout une cible chien ou page **par son adresse** (`get_page_by_path`, `:151`), à chaque requête ; une cible non résolue n'envoie rien (`service.php:94-95`), le visiteur tombe sur la 404. Les portées sont résolues par leur identifiant saisi, donc épargnées. Mesuré en transaction annulée par #59 : page 316 et chien 25 passent en `cible_non_resolue`, portée 165 suit. Une page, type hiérarchique, n'a même pas de `_wp_old_slug`. **Seule `wp mtb verifier-redirections` le voit. Aucune fiche `page-*.md` ne prévient l'éleveuse** ; la fiche chien le fait depuis `1fcf948`. **Préexistant à #59**, qui ouvre seulement un second chemin vers le champ. Déclarée T-#59-d | lot 22, #59, mesuré | **Q29** — puis une issue `seo` (résoudre par identifiant) ou `doc` (avertir dans les fiches page) |
| **T124** | **La liste Pages déborde à 360 px (415 px) quand la Modification rapide est ouverte** — la liste déroulante « Parent » du cœur, dont l'option la plus longue est « Publication principale (pas de publication parente) ». Fermée, la liste tient à 360. Préexistant : même largeur avec « Slug ». Déclarée T-#59-c, **corrigée par la passe d'intégration** (le contrat la disait ouverte en permanence) | lot 22, #59, mesuré | une issue `a11y` — liée à Q29 si la Modification rapide est masquée |
| **T123** | **La Modification rapide offre Mot de passe, Privée et État sans aucune des explications de l'écran complet** — un second chemin pour retirer un contenu du site ou le protéger. Préexistant à #59. Déclarée T-#59-b | lot 22, #59 | **Q29** |
| **T122** | **La liste Articles dit encore « Slug » dans la Modification rapide**, exclue délibérément par #59 (type non utilisé par l'éleveuse). Témoin utile : c'est elle qui prouve que la garde d'écran tient. Déclarée T-#59-a | lot 22, #59 | facultatif |
| ~~**T118**~~ | **✅ PAYÉE au lot 22 par #59** (`0379804`, `1fcf948`) : « Adresse de la page » dans la Modification rapide des listes Pages, Portées et Chiens, par un filtre `gettext_default` posé sur `load-edit.php` après garde d'écran — le cœur émet le mot en PHP (`_e('Slug')`), chemin que #54 n'avait jamais traité. Énoncé d'origine : **Le cœur émet « Slug » en PHP dans la Modification rapide, donc sur les listes Portées et Chiens — deux écrans quotidiens de l'éleveuse.** L'énoncé de #54 (« `page` est le seul type jamais traité ») était **plus optimiste que la réalité** : mesuré sur pièces, le cœur écrit « Slug » à **sept** endroits en PHP, dont `class-wp-posts-list-table.php:1708`, qui sert la Modification rapide de **toutes** les listes. Le module de #54 ne mord que sur l'écran d'édition d'une page : la Modification rapide reste en dehors, sur les trois types. **La chaîne a eu raison de ne pas s'élargir** — autre module, trois types, et un rename mesuré vaut mieux qu'un rename supposé. Déclarée **T‑#54‑b** | lot 21, #54, mesuré | une issue `contenu` — étendre le renommage à la Modification rapide des trois types |
| **T117** | **La garde `is_admin()` du rappel `the_author` porte le même défaut latent que celui qui a bloqué ce lot, à un rappel de distance.** `is_admin()` vaut **faux** sur `/wp-json/` — c'est la doctrine du dépôt, écrite au §4.3 de `issue-56.md`. Le rappel substituerait donc le titre du site au nom d'auteur dans un contexte REST d'édition, c'est-à-dire **ferait mentir une surface d'administration**. **Mesuré latent et non vivant** : zéro consommateur de `the_author` dans le thème et dans l'extension hors notre accroche ; les seuls appelants réels sont le cœur dans les trois gabarits de flux — **c'est-à-dire la cible même du correctif**. **Arbitrage du lot, à ne pas relire comme un oubli** : on ne met pas en jeu un livrable mesuré pour couvrir un cas dont on vient de mesurer qu'il n'existe pas. **Latent aujourd'hui, vivant le jour où quelque chose appellera `the_author` hors administration et hors flux.** Point gravé au §R15 : **la forme de repli ne peut pas être une capacité seule** — un flux est servi en amont de toute session, donc `/feed/` rendu à l'éleveuse connectée puis mis en cache par un frontal republierait le nom du compte à des anonymes ; seul le terme `REST_REQUEST` tient la forme composée. R15 **n'autorise rien** : la garde reste `is_admin()` seule tant qu'une issue datée ne la reprend pas | lot 21, #56, brainstorm puis addendum R15 | une issue `seo` — reprendre la garde en forme composée |
| **T116** | **L'oracle d'énumération subsiste sur `wp-login.php`, il a QUATRE discriminants et non un seul, et le plus fort n'a aucune couture dans le cœur.** Mesuré pour la première fois par #56 — **personne ne l'avait joué avant** : identifiant connu → « ce mot de passe ne correspond pas à l'identifiant **fabienne** » ; inconnu → « l'identifiant **zzz** n'est pas inscrit sur ce site ». **Mais le message n'est qu'un discriminant sur quatre** : (2) le champ `log` est repeuplé pour un identifiant connu et vidé pour un inconnu, piloté par le **code** d'erreur et non par le message ; (3) le script de focus vise `user_pass` ou `user_login`, même cause ; (4) **`?action=lostpassword` rend `302` vers `?checkemail=confirm` contre `200` + erreur — le signal le plus fort des quatre, et il ne passe par aucun message.** Le quatrième **n'a aucune couture** : `lostpassword_errors` court AVANT la garde `! $user_data` qui rajoute `invalidcombo` et rend la main, et il n'existe aucun filtre entre le retour de `retrieve_password()` et la décision de redirection. **Le fermer imposerait de réimplémenter un flux du cœur — la dette T114 créée volontairement, dans sa forme la plus coûteuse.** Quatre motifs de non-livraison, tous retenus : ne fermer que le message serait **une demi-fermeture rapportée comme entière** (interdit institué au §16 de #24, déjà payé deux fois) · hors empreinte de #56 · **c'est le seul volet du sujet qui change quelque chose qu'elle LIT, et au pire moment — le jour où elle n'entre pas chez elle** — donc **une fiche de guide est due au titre de D3**, et `docs/guide/` n'a aucune fiche de connexion sur ses 29 · et c'est **un arbitrage produit** entre la vie privée et l'autonomie de l'éditrice, qui n'appartient pas à une issue `seo` | lot 21, #56 §5, résidu R6 de `issue-49.md` §9 | une issue `prive`/`infra` portant **les quatre discriminants** et non le seul message, **avec sa fiche de guide** |
| **T115** | **Le mode de panne de `#49` et `#50` est totalement muet, et l'enjeu de l'un des deux ne l'est pas.** Si le rappel d'auteur cesse de mordre — accroche perdue dans une reprise, constante vidée, dossier renommé `_indexation-heritee`, cœur qui renomme une clé, tiers qui refiltre, ou un futur `set_current_screen()` / `define( 'WP_ADMIN', true )` sur une requête de front (LOW-4 de la revue, cause **absente** de la liste écrite dans le fichier) — alors `/author/<slug>/` **revient à 200 et rien ne le dit** : ni journal, ni écran, ni code de sortie. Et *une archive d'auteur qui répond 200 ressemble à une page qui marche*. **#50 protégeait une adresse machine que personne ne demande ; #49 protège l'identifiant de connexion de l'éleveuse.** Aucune commande WP-CLI n'est proposée : le **motif 3 gelé** en tête de `indexation-heritee/bootstrap.php` l'interdit — témoins d'échec disjoints d'avec `redirections-301`. Le seul témoin est un protocole rejoué à la main | lot 20, #49 §8 et #50 §8, revue LOW-4 | une issue `seo` ou `infra` |
| **T114** | **Les numéros de ligne du cœur de WordPress, cités dans trois fichiers et trois contrats, se périmeront EN SILENCE.** `wp-includes/` n'est pas versionné : les ancres relevées le 2026-09-08 sur **WordPress 6.9** — `class-wp.php:319-336, 409, 455-463, 566` · `wp-admin/includes/post.php:1319, 1407` · `class-wp-posts-list-table.php:317-336` · `class-wp-media-list-table.php:102, 510-517` · `upload.php:137, 140` · `class-wp-sitemaps.php:176-180, 200-202, 211-215` · `sitemaps.php:52-56, 132-151` · `canonical.php`, `template-loader.php` — ne seront vérifiées par rien. **La console d'administration annonce déjà « WordPress 7.1 est disponible »** : c'est cette montée qui les invalidera. Elles sont datées et leur version est écrite à côté, mais **aucun contrôle ne les relit** | lot 20, revue LOW-3, contrats #49 §14.5, #50 §14, #53 | ✅ **SOLDÉE au lot 22 par #57**, commits `5bbf54b` et `4d23c28`. **L'énoncé était faux sur deux points, et la mesure les a corrigés tous les deux** : ce ne sont pas « trois fichiers et trois contrats » mais **77 occurrences du nom du module dans 11 contrats**, et le périmètre retenu n'est pas la liste de sept ancres ci-dessus mais **les 77 fichiers du cœur cités quelque part avec un numéro de ligne**. **Ce qui est surveillé n'est pas la version mais l'empreinte md5 du fichier cité** : une 6.9.x qui ne touche aucun fichier cité ne déclenche donc rien, et la version ne sert que de contexte. Le témoin tourne à chaque provisionnement et `docker-mtb` recopie sa ligne de bilan en fin de lot |
| **T113** | **L'énumération des comptes reste ouverte par la route REST, et cette fuite est PLUS RICHE que celle que #49 vient de fermer.** Mesuré en anonyme, sans cookie : `/wp-json/wp/v2/users` → **200 · 672 o**, **les deux comptes d'un seul coup**, `"slug":"admin"`, `"slug":"fabienne"`, **`"name":"Fabienne Guéneau"` — le nom civil de l'éleveuse** ; `/wp-json/wp/v2/users/1` → 200 · 335 o, `/users/2` → 200 · 334 o, `/users/3` → **404 · 90 o**, donc **l'oracle par identifiant y est intact, exactement la mécanique que #49 ferme sur `?author=N`** ; `/feed/` → `<dc:creator><![CDATA[admin]]></dc:creator>`, soit **l'identifiant de connexion** du compte 1 et non un nom d'affichage. **Trois faits à ne pas perdre** : la liste rend l'oracle par identifiant *superflu* plutôt que de le reproduire ; **le front refuse désormais de confirmer un slug que la REST publie** ; et **R6 — `wp-login.php`, dont le message diffère entre identifiant connu et inconnu — n'a été mesuré par personne**, alors qu'il protège le même bien. Résidus R1, R2, R4, R6 du §9 de `docs/contracts/issue-49.md`, **déclarés et délibérément non fermés** : #49 a eu raison de ne pas s'élargir seule à la REST | lot 20, #49 §9, passe d'intégration §8, revue | ⚠️ **SOLDÉE AUX TROIS QUARTS au lot 21 par #56**, commits `9dcf04c` → `22bd316`. **Fermé et mesuré** : les neuf adresses d'identité rendent en anonyme `404 · 183 o · md5 4015aeea…`, **identiques au md5 près, existant et inexistant confondus** — l'oracle par identifiant, par recherche et par la seconde écriture `?rest_route=` meurent ensemble ; zéro identité sur **douze** surfaces, oEmbed **XML** compris ; `_embedded.author` rend `rest_no_route`. **R3 est entré dans le périmètre par la mesure** : #49 §9 le classait inoffensif sur la foi de l'accueil seul, alors que **tout contenu singulier** publiait `"author_name":"admin"`. **R6 — `wp-login.php` — reste ouvert, mesuré et non corrigé** : voir T116 |
| **T112** | **Le guide dit « pages » pour des contenus qui n'en sont pas, et ce lot ne l'a corrigé que là où ça trompait sur un geste.** Quatre des cinq contenus tenus à l'écart des moteurs sont des **fiches de chien**, pas des pages — donc « remettez **la page** dans un menu » l'envoie chercher Halan dans **Pages**, où il n'est pas. Le lot 19 a corrigé le H1, le titre de section et les endroits qui **décrivent un geste** ; il reste des occurrences qui **ne trompent pas** (`contenu-a-l-ecart:99` « L'une des cinq », juste par accident puisque Placement est une page) et des occurrences **voulues** qu'il ne faut surtout pas toucher (`:40`, `:47`, `:53`, dans la section où « pages » est **porteur du raisonnement** sur les liens entre pages, et qui porte le solde de la décision 72 ; `:88`, libellé de menu). **Le danger de cette dette est sa correction mécanique** : un `sed` sur « pages » abîmerait précisément les trois occurrences justes. Se solde par une passe qui relit les 28 fiches et arbitre **occurrence par occurrence**, avec le critère « cette phrase décrit-elle un geste ? » **Second cas de la même famille, relevé à la clôture** : `menu-modifier-le-menu.md` ne documente que l'encadré **Pages**. Le jour où elle voudra poser **Halan** dans un menu — geste que le lot 19 vient précisément de lui rendre en corrigeant « dites-le-nous » — cette fiche **ne lui montrera pas l'encadré Chiens**. Le guide lui dit donc de faire seule un geste qu'il ne lui décrit qu'à moitié | lot 19, revue (LOW-5) et chaîne #52 | ✅ **SOLDÉE au lot 21 par #55**, commits `a6ec9a9` → `85d1219`. **Le critère de l'énoncé a été remplacé après mesure** : « cette phrase décrit-elle un geste ? » se trompait dans les deux sens, remplacé par un test à deux détentes appliqué au paragraphe (porte/prépare un geste ? enverrait dans le mauvais encadré ?). **25 fiches sur 29 sortent intactes — résultat attendu, pas un manque.** `menu-modifier-le-menu.md` documente désormais l'encadré **Chiens**, et sa section neuve **commence par Options de l'écran** parce que la mesure a montré que les encadrés Chiens et Portées sont livrés **décochés** — une fiche disant « l'encadré Chiens est dans la colonne de gauche » aurait été fausse à l'écran, **T112 recréée par sa propre correction**. Trois défauts rattrapés avant commit : un **inventaire gravé** (« vos 22 chiens… il n'en montre que 15 », faux au 23ᵉ et rien ne l'aurait signalé), un **motif inventé** sur les résultats de travail, un **repère faux** (« sous Pages », Articles s'intercalant) |
| **T111** | **`ENDORMI` et `REVEILLE` sont recopiés en littéraux dans `editeur.js`, seule des quatre recopies du module à ne pas être fermée.** L'arbitrage A9 a fermé trois recopies — `TYPES`, `CHAMP`, et la clé transmise au JavaScript, cette dernière **gardée** (sans clé exploitable le panneau ne se monte pas du tout : « un panneau absent se voit ; un panneau qui écrit sous une clé vide ne se voit pas »). Mais `editeur.js:96` teste `'1' === meta[ CLE ]` et `:172` écrit `coche ? '1' : '0'`, alors que les deux constantes existent en PHP (`etat.php:49,54`) et n'ont pas été jointes à la charge `wp_json_encode()`. **Mode de panne s'il se réalisait** : `ENDORMI` changé en PHP, le JavaScript continuerait d'écrire `'1'`, `est_en_sommeil()` ne le reconnaîtrait plus, **la case reviendrait décochée à la réouverture, la page répondrait 200 et le journal resterait vide**. **Risque pratique quasi nul** — ce sont les deux états d'un booléen, gelés au §2 du contrat et interdits de changement au §12. **Ce que la revue relevait était le silence, pas le risque** : A9 a déclaré nommément les recopies laissées, celle-ci ne l'était pas. **Le silence est levé** — déclarée dans A9 au lot 19, avec son motif : la fermer imposerait de rouvrir `editeur-de-blocs.php` et de **rejouer E4 au navigateur**, seule sonde prouvant que le panneau écrit | lot 19, revue (LOW-1) | l'issue qui rouvrira le module, si elle rejoue E4 de toute façon |
| **T110** | **L'écran d'une page montre « Slug » et « Définir l'image mise en avant », deux mots que `MASTER.md` §10.2 interdit nommément.** Et la fiche du sommeil les **recopie honnêtement**, donc elle apprend le mot « Slug » à l'éleveuse (`contenu-mettre-en-sommeil-et-reveiller.md:59` et l'`alt` de `:70`). **L'imputation est le cœur de cette dette et elle a été arbitrée** : ce n'est **pas la fiche** — elle décrit ce que l'éleveuse voit, et la « corriger » la ferait mentir, exactement l'erreur qui a rendu HIGH-2 bloquant ; ce n'est **pas `MASTER.md`** — §10.2 a raison d'interdire ces mots, puisqu'elle ne doit jamais en avoir besoin, rien à rouvrir ni à remonter à l'utilisateur ; ce n'est **pas le cœur de WordPress** — ses chaînes sont renommables et **ce dépôt sait déjà le faire** : `admin/description-photo` renomme « Texte alternatif » par un rappel `gettext`, `fields/portee/ecran.php` et `fields/chien/bootstrap.php` remplacent la boîte d'adresse du cœur sur les deux types `mtb_`. **C'est un trou de couverture de `mtb-core` sur le seul type jamais traité, `page`** | lot 19, revue (MEDIUM-4) | ✅ **SOLDÉE au lot 21 par #54**, commits `f462817` → `2cf8314`. **La prémisse de l'issue était fausse, et la chaîne l'a démontrée sur mesure plutôt que de l'appliquer** : « étendre le rappel `gettext` » n'atteint **ni l'un ni l'autre** des deux libellés, et pas de la même façon. « Définir l'image mise en avant » passe par `post_type_labels_page` — un filtre dynamique **dont le nom est la borne**, donc Portée et Chien sont hors d'atteinte *par construction*, et qui **ne compare aucune chaîne**, donc insensible à une reformulation du cœur. « Slug » n'est émis par **aucun PHP** sur cet écran : `__("Slug")` nu du paquet `wp-editor`, traité par un rappel `i18n.gettext` en JavaScript sous garde d'écran serveur. La garde `is_admin()` du modèle `description-photo` a été **mesurée nuisible** et interdite : zéro occurrence dans le module. **L'écran de l'éleveuse est couvert, page d'accueil comprise** (voir décision 82) |
| **T109** | **Plus aucun témoin rejouable ne prouve que les contenus repris restent hors des moteurs — seulement qu'ils portent encore la clé.** L'égalité des trois nombres du §6.2 de #24 était ce témoin ; #52 l'a légitimement remplacée (relève au §11.3 de son contrat), mais **le seul contrôle automatisé restant, `wp mtb verifier-redirections` étape 6 (`CONTENUS_NOINDEX_ATTENDUS`), compte `_mtb_robots_source` EN BASE et reste VERT même si `convertir()` n'a jamais couru.** La preuve de comportement est le protocole C1→C6, **manuel et à usage unique**. Le module l'écrit lui-même : « les contenus repris ne sont jamais convertis et restent indexables — la page répond 200, le journal reste vide ». **Vrai sur la base d'aujourd'hui, muet sur toute base restaurée par copie** — donc muet exactement le jour où ça compterait. Remède nommé par la revue : **une étape de plus à la commande existante**, mesurant l'effet et non la présence de la clé | lot 19, revue (MEDIUM-7) | ✅ **SOLDÉE au lot 20 par #53**, commits `2574bf9` et `c203fc1`. Une **étape 6 bis** nichée rend un verdict par contenu, mesure l'effet sur le plan du site fabriqué **en processus**, et **attribue** cet effet à notre rappel par une sonde de contraste. **La preuve est la plus forte des deux** : dans une **seule** exécution, l'ancienne étape 6 imprime « 5 (attendu 5) » — verte — pendant que l'étape 6 bis échoue en nommant les cinq contenus. Trois cas rouges obtenus par **deux méthodes indépendantes** (`--require` du dev, `eval-file` + `runcommand(launch=>false)` de la chaîne) : `conversion_jamais_jouee`, `rappel_non_accroche`, `effet_absent` |
| **T108** | **L'éleveuse ne peut plus rallumer une page qu'elle éteint — et c'est un geste qu'elle fait déjà.** `indexation-heritee/robots.php` le déclare lui-même : « un `noindex` posé par filtre est **invisible et irréversible depuis `wp-admin`** ». Tant qu'on croyait ces cinq contenus retirés pour de bon, le trou était théorique. **Il ne l'est plus** : interrogée le 2026-09-05, l'éleveuse dit de ces pages qu'elles sont « **en sommeil** » et, textuellement, « **placement je l'affiche parfois** ». Ce n'est donc pas un état figé qu'on reprend, c'est un **geste périodique**, et le site livré le lui retire — **le nouveau site lui en offre moins que l'ancien sur ce point précis**, IONOS lui donnant la case correspondante. Violation de la **contrainte 1**, jugée **HIGH** par la revue du lot 18 et **levée uniquement par le guide**, qui dit désormais la vérité : afficher Placement ne la ramène pas dans les moteurs, rien à l'écran ne le signale, un écran est prévu. **Le mécanisme à livrer existe déjà à 90 %** : « en sommeil » tel qu'elle le vit — hors moteurs, hors plan du site, hors recherche interne, mais la page s'ouvre encore avec le lien — est **exactement** ce que `query/page-protegee/` livre pour le contenu protégé, **sans le mot de passe**. Il reste à poser l'interrupteur. **Porte sur les TROIS types — portée, chien et page** (décision 74) : c'est **une archive réversible à tout moment**, et l'éleveuse seule décide de ce qu'elle y met — restreindre les types déciderait à sa place. Une portée passée est le cas d'archive le plus évident d'un site d'élevage | lot 18, fait d'élevage recueilli après le gel des deux contrats | ✅ **PAYÉE au lot 19 par #52**, et **vérifiée sur le code, pas sur les rapports** : le seul mécanisme qui rendait les cinq contenus irréversibles a été **supprimé et non doublé** (`robots.php` absent, `ecarter_les_noindex()` retirée), et `convertir_un()` **ne peut pas défaire un réveil** puisqu'il sort sur `metadata_exists()`. Le va-et-vient a été joué **au navigateur, au clic sur la vraie case**, pour les **trois** types et sur les **deux** façades : le réveil retire le `noindex` **et ramène le contenu au plan du site** (18 → 19 chiens sur Halan), tandis que `_mtb_robots_source` reste intacte avec sa provenance. Le troisième état de la méta — **clé absente** ≠ « réglé à non » — est ce qui rend la conversion ré-exécutable **sans jamais rendormir ce qu'elle a réveillé** : `int(5)` puis `int(0)`, et un contenu remis à `'0'` **reste** `'0'` |
| **T107** | **L'inventaire du §11 du contrat #1 porte 12 lignes pour 34 modules sur le disque : 22 manquent.** Les 13 de `blocks/`, les 5 de `admin/`, `query/coordonnees`, et les 3 de `migration/` livrés avant le lot 18. La cause est tranchée par la **décision 70** (le lead tient la table, pas les chaînes) et l'avertissement daté est posé sous la table ; **reste à payer le recensement lui-même**. Ce n'est pas cosmétique : le §11 est la **seule carte du plugin**, puisque le contrat #1 a délibérément écarté tout index central dans le code — une chaîne qui cherche si un module existe et lit son silence conclura qu'il n'existe pas. Se solde par une passe qui relit les 22 `bootstrap.php` et écrit leur rôle **depuis le code**, pas depuis les énoncés d'issues | lot 18, décision 70 | une passe de recensement, ou l'issue `doc` qui la portera |
| **T106** | **Retirer le fournisseur `users` du plan du site laisse un faux 404.** `/wp-sitemap-users-1.xml` répond **200 en `text/html`**, 16 035 o, corps quasi vide — là où un sous-plan réellement absent, `/wp-sitemap-posts-mtb_resultat-1.xml`, rend un **404 franc**, et où `/nexiste-pas-du-tout/` rend 18 284 o de « Page introuvable ». Un *soft-404* est ce que les moteurs classent le moins bien : ils le tiennent pour une page réelle et vide. **Découvert par la passe d'intégration du lot 18, non déclaré par #24**, constaté et non corrigé. Sans conséquence tant que le site n'est pas en ligne | lot 18, passe d'intégration | ✅ **SOLDÉE au lot 20 par #50**, commit `5c69ca5`. Un rappel `template_redirect` **20** pose `set_404()` + `status_header(404)` — priorité choisie pour passer après `render_sitemaps()` **et** après `redirect_canonical`, dont le devineur de 404 aurait sinon transformé la réponse en 301 vers un contenu deviné. Mesuré : `200 text/html 16035` → **`404 text/html 18284`**, identique à l'octet à `/wp-sitemap-posts-mtb_resultat-1.xml`. **Résidu assumé, borné plus finement que prévu** : seule la classe « nom de fournisseur **non enregistré** » (`usersx`, `zzz`) garde un faux 200 — un sous-type inconnu d'un fournisseur **enregistré** rend bien un 404 franc. Borne 3 du contrat, à ne pas lire comme un oubli |
| **T105** | **L'énumération des auteurs reste ouverte, même si le plan du site ne la publie plus.** #24 a mesuré la fuite (`user_nicename` = `user_login` = `admin`, l'identifiant de connexion de l'administrateur publié à Google — BRIEF §4) et l'a fermée côté plan du site en retirant le fournisseur `users`, exception motivée et datée à son contrat. **Le résidu est déclaré, pas masqué** : `/author/admin/` répond toujours **200**, et `/?author=1` y redirige en **301** — donc l'identifiant reste devinable par quiconque le cherche. Le fermer suppose de neutraliser la requête d'auteur ou ses règles de réécriture : **ni un héritage de l'ancien site, ni dans les sept tâches de #24**, qui a eu raison de ne pas l'élargir seule. ⚠️ **L'énoncé ci-dessus est daté du lot 18 et n'est plus vrai depuis `325c6be`** : `/author/admin/` et `/?author=1` rendent désormais 404 | lot 18, #24 | ✅ **SOLDÉE SUR SON ÉNONCÉ au lot 20 par #49**, commits `325c6be`, `8b99687`, `1c073ec`. La requête d'auteur est neutralisée dans le filtre `request` 10 — clés retirées, `error = '404'` — donc jamais une réponse posée après coup. **Le périmètre livré dépasse l'énoncé, sur mesure et non par zèle** : il y avait **deux** comptes et non un, dont `fabienne`, celui de l'éleveuse ; s'en tenir à la lettre aurait laissé publié **son** identifiant de connexion. Deux formes non nommées fermées aussi (`?author_name=`, `/author/<slug>/feed/`). Mesuré : les seize formes rendent `404 · 18 284 o · md5 d9f9bae7`, **aucun `Location`**, corps indiscernable d'un 404 sans rapport. ⛔ **MAIS LA DETTE N'EST PAS SOLDÉE DANS SON ESPRIT — voir T113**, plus riche que celle-ci |
...
| **T88** | **48 citations `functions.php:NNN` dans le dépôt, aucune exacte** (R10 du contrat #40, déclarée à `issue-40.md:751-800`). #40 a décalé de +11/+18 lignes après remède ; 8 sont dans des sources CSS que son propre gel interdisait d'ouvrir, 2 dans `docs/ETAT.md`, 15 dans des contrats gelés, 1 dans `blocks/lien-de-recours/rendu.php:227`. **Mais 7 étaient déjà fausses à `e8a35f4`** : la pratique de citer ce fichier par numéro se dégradait déjà seule — c'est un problème de **méthode**, pas un dégât de #40. Table complète citant → cible → nouveau numéro au contrat ; la reprise est mécanique. **Aggravée le 2026-09-18 par #57**, qui n'ajoute que des commentaires mais décale de 6 à 43 rangs tout ce qui les suit dans sept fichiers PHP : `blocks/lien-de-recours/rendu.php:227` et `fields/portee/ecran.php:566`, cités dans ce fichier, ne visent plus leur cible, et le bloc des motifs gelés d'`indexation-heritee/bootstrap.php`, cité « `:17-33` », court désormais jusqu'à la l. 62 environ. **Les numéros ne sont pas réécrits ici, et le décalage ne se rattrape pas par addition** : la mesure montre que `mtb-portee-galerie` est aujourd'hui à la l. **591**, quand `566 + 11` tomberait sur une ligne de commentaire — ces citations étaient **déjà fausses avant #57**, ce qui est exactement l'énoncé de cette dette. La reprise reste une passe d'alignement, sur mesure et jamais par arithmétique | lot 14, #40 ; aggravée lot 22, #57 | une passe d'alignement, ou au fil de l'eau |
...
|-----|------|--------|----------|--------|
| 22 (1re partie) | 15. Dette technique — oracle REST | #59 | **T118 payée** : « Adresse de la page » dans la Modification rapide des listes Pages, Portées et Chiens. Le mécanisme a été mesuré avant le crochet : `_e('Slug')` en PHP (`class-wp-posts-list-table.php:1708`), filtre `gettext_default` posé sur `load-edit.php` après garde d'écran ; Articles volontairement exclue (T122). Intégration **104/104** (2 échecs imputables au test, corrigés). Revue **BLOQUANTE au premier passage — 0 CRITICAL, 1 HIGH** (phrase d'unicité de la fiche chien, décision 86), levé par `1fcf948`, puis **OK avec réserves**. Mesures faites en transaction annulée : l'adresse vide est réécrite de la même façon par la Modification rapide, et **T125** a été découverte (301 de l'ancien site coupée en silence). Q29 posée. Docker Desktop était arrêté au démarrage : relancé, volume conservé | `0379804`, `1fcf948` |
| 21 | 14. Dette technique — écran de page et guide + 15. Dette technique — oracle REST | #54, #55, #56 | **Le lot où trois chaînes ont chacune refusé la prémisse de leur propre issue, sur mesure, et avaient raison les trois fois.** #54 démontre que « étendre le rappel `gettext` » n'atteint **aucun** des deux libellés et livre deux mécanismes distincts (T110 soldée). #55 remplace le critère de son énoncé par un test à deux détentes et **laisse 25 fiches sur 29 intactes** (T112 soldée). #56 ferme l'oracle REST, les flux et l'oEmbed — **neuf adresses anonymes au même md5, existant et inexistant confondus** — après avoir été **renvoyée au travail par le lead** : sa première livraison faisait afficher « (Aucun auteur/autrice) » à tout écran d'édition, le mode de panne de la décision 79 reproduit sur le module qui venait de le payer. **Le défaut a été trouvé par une chaîne sœur qui refusait de photographier une régression pour le guide client**, pas par une sonde. Revue **BLOQUANTE au premier passage — 0 CRITICAL, 1 HIGH** : deux fichiers livrés affirmaient au présent que la seule parade contre une panne muette **était** une ligne du guide, **qui n'existait dans aucune des 29 fiches** — tombée entre deux empreintes que le lead avait découpées. Levé, revérifié. 3 MEDIUM, 3 LOW, dont un résidu où un contrat **se déclarait corrigé sans l'être**. Intégration : **58 contrôles, 56 conformes, 0 échec imputable au lot** | `9dcf04c` → `85d1219` |
| 20 | 12. Dette technique — indexation héritée + 14. Dette technique — témoin | #50, #53, #49 | **Le lot où un défaut a échappé à la chaîne, à la passe d'intégration ET à ma première vérification, et où il touchait l'éleveuse en propre.** #50 pose le 404 franc du sous-plan `users` retiré (`200/16035` → `404/18284`, T106 soldée). #53 remplace l'étape 6 de `verifier-redirections`, verte même sans conversion, par une étape 6 bis qui mesure l'**effet** — preuve donnée **côte à côte dans une seule sortie** : l'ancienne étape imprime « 5 (attendu 5) » pendant que la nouvelle échoue en nommant les cinq contenus (T109 soldée). #49 ferme l'énumération des auteurs — et **la mesure a rendu deux comptes là où mon énoncé en annonçait un**, dont `fabienne`, celui de l'éleveuse : s'en tenir à ma lettre aurait laissé publié **son** identifiant de connexion. **Revue BLOQUANTE** : le filtre `request` courait aussi en administration, `edit.php?…&author=2` rendait 404 en affichant les 33 portées sous un onglet « À moi (1) » — un écran qui a l'air de marcher et qui ment. Corrigé par une ligne (`is_admin() ||`). **T105 close sur son énoncé seulement : T113 la remplace et elle est plus grave** — `/wp-json/wp/v2/users` publie les deux comptes et le nom civil de l'éleveuse. ~356 vérifications, 355 conformes, 0 échec du code | `5c69ca5` → `1c073ec` |
| 19 | 10. Guide — vue d'ensemble + 13. Sommeil et réveil | #25, #51, #52 | **Le lot où le seul défaut de code a été trouvé par une capture d'écran, pas par les 87 vérifications d'intégration** — l'avertissement se rendait à côté de la case et non dessous, `PluginPostStatusInfo` enveloppant ses enfants en `flex-direction:row`. Sommeil et réveil livrés sur les trois types, méta à **trois états** (`'1'`, `'0'`, clé absente) qui rend la conversion ré-exécutable sans jamais rendormir un contenu réveillé. §11 du contrat #1 recensé : 37 modules, 37 lignes. 87 vérifications, 84 conformes, 0 échec imputable au code ; revue bloquante au premier passage, 2 HIGH documentaires | `b5aabd5` → `4bc8daa` |
| 18 | 9. Contact, page protégée, SEO/redirections + 11. Dette technique | #23, #24, #42 | **Le lot où le fait qui comptait le plus n'était dans aucun document : il est venu de l'éleveuse, après le gel des deux contrats.** Interrogée sur les cinq pages que l'ancien site retire des moteurs, elle répond qu'elles sont « **en sommeil** » et — textuellement — « **placement je l'affiche parfois** ». Ce n'était donc pas un état figé qu'on reprenait, mais un **geste périodique**, et le site livré le lui retirait : dette **T108**, HIGH de revue, levé par le guide seul. **Trois issues, sept commits, zéro échec de code sur 142 vérifications.** **#23** livre 18 lignes de code utiles — trois crochets, aucune fonction globale, aucune option, aucune méta — et tranche son domicile en `query/` **contre** `admin/` sur une asymétrie de danger : `admin/listes/bootstrap.php:56-63` pose sa garde d'exclusion **à propos d'un `pre_get_posts`**, donc le module `admin` le plus proche avait déjà tranché contre ce domicile, sur le hook exact ; une impureté de forme s'objecte en revue, **une mort silencieuse ne se dit nulle part**. **#24** rend les 52 adresses de l'ancien site, **vérifiées une à une en 82 mesures** (46 × 301→200, 5 identiques, 1 × 404 voulu), et **ne préjuge d'aucun schéma de permalien** comme la décision 69 l'exigeait — `carte.php` ne stocke **aucune URL**, seulement des identités `(type, référence)` résolues par `get_permalink()` à chaque requête. Elle trouve et ferme une fuite que personne n'avait cherchée : le plan du site publiait `/author/admin/`, `user_nicename` = `user_login` = `admin`, soit **l'identifiant de connexion de l'administrateur livré à Google** (BRIEF §4) — mesurée avant d'être conclue, résidu déclaré (T105). **#42** aligne `MASTER.md` en 1.6 et **refuse d'écrire « neuf » dans le guide** : `page-ce-qui-a-ete-repris…` compte les disciplines **peuplées**, `truffe` en ayant zéro — neuf dans la liste, huit à l'écran, deux comptes légitimes, et « harmoniser » aurait fabriqué l'erreur (même figure que T99 au lot 16). **Le mode de panne qui a ouvert le lot était dans les énoncés eux-mêmes** : #23 et #24 déclaraient des empreintes sous `includes/privacy/` et `includes/seo/`, **ni l'un ni l'autre n'étant un des six groupes du chargeur** — un `bootstrap.php` posé là n'aurait **jamais été inclus, sans erreur ni ligne au journal**. Corollaire vérifié à la clôture : sous un groupe **connu**, le chargeur journalise bien « ignoré : aucun bootstrap.php lisible » ; sous un groupe **inconnu**, il ne balaie rien et n'écrit rien. **La jonction que ni #23 ni #24 ne pouvait prouver seule est prouvée** : deux rappels sur `wp_sitemaps_posts_query_args`, **additifs**, comptes exacts au chiffre près (`page` 9→7, `mtb_chien` 22→18, `mtb_portee` 33→32, total #24 = 5) ; la convention que j'ai gelée en cours de lot — **muter des clés, jamais remplacer `$args`** — est tenue dans le code **et** dans le résultat. Revue **BLOQUANTE au premier passage — 0 CRITICAL, 2 HIGH, tous deux de texte**, levés avant le push : la fiche promettait à l'éleveuse que « la décision vous appartient entièrement » alors qu'elle n'a **aucun écran**, et érigeait une corrélation en cause. **Le second HIGH est ma faute** : j'avais transmis la mesure des cinq pages en écrivant « la règle réelle de l'ancien site », franchissant le pas que Q23, la décision 52 et `RELEVE.md` interdisent — décision **72**. **La leçon du lot est celle que l'utilisateur a formulée** : avant de lui remonter une question, chercher si le dépôt la tranche déjà. Ma question sur la page Placement a coûté trois mesures pour finir en « ça ne change rien » — `docs/migration/source/` portait la réponse entière, et le vrai renseignement est venu de l'éleveuse, pas de moi | `05000d1`, `403091c`, `da1d029`, `a765abf`, `9d2acee`, `25ebd80`, `ff3e3fc` |
...
| 17 | — | — | **Ligne manquante, constatée à la clôture du lot 20 et non reconstituée.** Le lot 19 manquait aussi ; ses données figuraient dans la section « Où on en est » de ce fichier, il a donc été rétabli. **Celles du lot 17 n'y sont pas** : le journal est tenu à la clôture, et cette clôture-là ne l'a pas alimenté. À reconstituer depuis `git log` par qui en aura besoin — **ne pas l'inventer** | — |
