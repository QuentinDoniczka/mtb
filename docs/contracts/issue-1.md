...
**Compte au 2026-09-07 : 34 modules** — `content` 3 · `fields` 3 · `query` 5 · `blocks` 13 · `admin` 5
...
**Une chaîne (#52) travaillait sous `wp-content/plugins/mtb-core/includes/` pendant ce recensement.**
Le module qu'elle livrerait éventuellement **n'est pas dans ce compte** : il n'est ici ni nommé, ni
classé, ni décrit, parce qu'on ne décrit pas un module qu'on n'a pas lu.
...
| `migration/` | `indexation-heritee` | #24 | `wp_robots` « noindex, nofollow » et retrait du plan du site pour les contenus portant `_mtb_robots_source` (5 aujourd'hui) ; retrait du fournisseur `users` du plan du site (exception motivée au §6.4, datée du 2026-09-05). La liste des identifiants n'est **jamais** codée en dur : le filtre lit la méta, écrite par les deux imports. | `wp_robots` **20** · `wp_sitemaps_posts_query_args` 10 · `wp_sitemaps_add_provider` 10. Aucune garde `WP_CLI`, aucune commande, aucune fonction globale. **Amendement déclaré dans le module**, désigné comme « le même amendement que le module voisin » `redirections-301` (lecture seule, aucun état en base pour se déclencher, périmètre clos et daté). |
...
> Dernière mise à jour : **#51, le 2026-09-07** — recensement complet des 34 modules depuis leur code.
