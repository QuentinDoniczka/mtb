...
| `migration/` | `redirections-301` | #24 | Table gelée des 52 adresses de l'ancien site ; 301 sur `template_redirect` 1 ; réparation des ancres internes sur `the_content` 20 ; commande `wp mtb verifier-redirections`. Amendement au §2 déclaré dans `docs/contracts/issue-24.md`. Le module est **en lecture seule** et ne dépend d'aucun état en base pour se déclencher. | `template_redirect` **1** · `the_content` **20**, tous deux **sans garde `WP_CLI`** — ce sont des services de front. Sous garde `WP_CLI` seulement : `WP_CLI::add_command( 'mtb verifier-redirections' )`. Aucune fonction globale. |
...
> Dernière mise à jour : **clôture du lot 19, le 2026-09-07** — recensement complet des 34 modules
...
> Mise à jour précédente : clôture du **lot 18** (issues #23, #24 — #42 n'a livré aucun module).
>
