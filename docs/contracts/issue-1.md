...
## 11. Inventaire des modules

C'est la carte du plugin, puisque le code n'en contient aucune.

**Qui la tient.** Le §11 est tenu par `/lead-mtb`, à la clôture de chaque lot, et **jamais par une
chaîne** : ce fichier est hors de l'empreinte de toute chaîne, et trois chaînes qui écriraient la même
ligne sur un arbre de travail unique s'écraseraient mutuellement, en silence, dernier arrivé gagnant.
Ce document se contredisait sur ce point — les l. 12-14 (« chaque issue qui livre un module l'y
ajoute ») contre la note de tenue placée sous la table. La contradiction est **tranchée en faveur de la
seconde lecture par la décision 70 de `docs/ETAT.md`**. **L'énoncé gelé des l. 12-14 n'est pas
réécrit** (décision 65) ; son erratum est daté dans `ETAT.md` et **prime sur lui** (décision 66). Cette
règle de propriété n'est pas datée : elle survit à la dette qui l'a révélée.

**Ce que dit la colonne « Livré par ».** L'issue qui a **créé** le module, établie par le premier
commit ayant ajouté son `bootstrap.php`. Ce n'est **pas** l'issue qui le gouverne aujourd'hui :
plusieurs modules ont été réécrits depuis par des issues ultérieures, et les amendements de contrat qui
les concernent sont nommés dans la colonne « Accroches ».

**Compte au 2026-09-07 : 37 modules** — `content` 3 · `fields` 4 · `query` 6 · `blocks` 13 · `admin` 6
· `migration` 5. Il se recompte en trois secondes, sans avoir à croire personne :

```
find wp-content/plugins/mtb-core/includes -name bootstrap.php | wc -l
```

**Règle de falsification, permanente.** Si le compte du disque diffère du compte de cette table, **la
table est périmée** : son silence ne prouve alors plus rien, et **aucune chaîne ne doit conclure de
l'absence d'une ligne qu'un module n'existe pas.** Cette règle ne se solde pas et ne se date pas — elle
se relit à chaque consultation, et elle est vraie exactement quand sa condition l'est.

**La réserve posée par #51 est levée.** Elle avait recensé 34 modules en signalant qu'une chaîne (#52)
travaillait sous `includes/` pendant sa passe, et avait refusé de décrire un module qu'elle n'avait pas
lu — c'était le bon refus. #52 a depuis livré ses **trois** modules : `fields/sommeil`,
`query/mise-en-sommeil` et `admin/sommeil`. Ils sont ajoutés ici **à la clôture du lot 19, par le
lead**, décrits depuis leur code et non depuis l'énoncé de l'issue. La ligne `migration/indexation-heritee`
est **réécrite dans le même geste** : #52 lui a retiré ses deux directives d'indexation, et la décrire
encore comme les posant aurait fait de cette carte un faux.

| Groupe | Module | Livré par | Rôle | Accroches |
|---|---|---|---|---|
| `content/` | `chien` | #4 | Enregistre le type de contenu `mtb_chien` — adresses en `/chien/<nom-d-usage>/`, aucune archive, le titre WordPress **étant** le nom d'usage — ses clés de méta `_mtb_` avec assainisseur et `auth_callback` explicites, et impose le formulaire classique à sa fiche. | `init` 10 → `register_post_type( 'mtb_chien' )` · `init` 10 → `register_post_meta()` (listes de `choix.php`, assainisseurs de `assainissement.php`) · `use_block_editor_for_post_type` 10 · `enter_title_here` 10. Aucune fonction globale. |
| `content/` | `portee` | #3 | Enregistre le type `mtb_portee` — archive `/portees/`, `with_front` faux — puis, dans le même rappel, ses 16 clés de méta `_mtb_`, leurs listes fermées et leurs assainisseurs. | `init` 10 → `register_post_type( 'mtb_portee' )` puis 16 `register_post_meta()`. Aucun autre crochet, aucune fonction globale. |
| `content/` | `resultat` | #5 | Enregistre le type `mtb_resultat`, **sans aucune page publique** (le titre est composé au serveur), et ses 8 clés de méta `_mtb_`. | `init` 10 → `register_post_type( 'mtb_resultat' )` puis 8 `register_post_meta()`. Aucun autre crochet, aucune fonction globale. |
| `fields/` | `chien` | #4 | L'écran de saisie de la fiche Chien : six sections — Identité, Parents, Taille et robe, Santé, Titres et brevets, Photos et pedigree — plus « Adresse de la page » qui **remplace** la boîte du cœur, la sauvegarde, les avis et les messages d'enregistrement en français d'élevage. | `init` 20 (poignées `mtb-chien-statut`, `mtb-chien-galerie`) · `add_meta_boxes` 10 · `edit_form_after_title` 10 · `save_post_mtb_chien` 10 · `admin_enqueue_scripts` 10 · `admin_notices` 10 · `redirect_post_location` 10 · `wp_editor_settings` 10 · `mce_buttons_2` 10 · `post_updated_messages` 10 · `bulk_post_updated_messages` 10 · `get_sample_permalink_html` 10 ; `remove_meta_box( 'slugdiv' )`. Aucune fonction globale. |
| `fields/` | `portee` | #3 | L'écran de saisie d'une portée : quatre boîtes — La portée, Père et mère, Les chiots, Galerie photos — plus « Adresse de la page » en remplacement de celle du cœur, la sauvegarde, les avis portés par transient jusqu'à l'écran qui suit la redirection, et les messages d'enregistrement. | `add_meta_boxes` 10 · `edit_form_after_title` 10 · `save_post_mtb_portee` 10 · `init` 20 (poignée `mtb-portee-ecran`, `ecran.js`) · `admin_enqueue_scripts` 10 · `admin_notices` 10 · `use_block_editor_for_post_type` 10 · `enter_title_here` 10 · `get_sample_permalink_html` 10 · `redirect_post_location` 10 · `post_updated_messages` 10 · `bulk_post_updated_messages` 10 · `mce_buttons_2` 10 · `wp_editor_settings` 10 ; `remove_meta_box( 'slugdiv' )`. Aucune fonction globale. |
| `fields/` | `resultat` | #5 | L'écran de saisie d'un résultat de travail en une seule boîte, son contrôle « chien : fiche ou nom libre », le titre composé au serveur à l'enregistrement, et les messages qui empêchent le cœur d'annoncer « Article publié ». | `add_meta_boxes` 10 · `save_post_mtb_resultat` 10 · `post_updated_messages` 10 · `bulk_post_updated_messages` 10 ; `remove_meta_box( 'slugdiv' )`. Aucune fonction globale. |
| `fields/` | `sommeil` | #52 | L'interrupteur « en sommeil » sur l'écran de saisie des **trois** types de `TYPES` — `page`, `mtb_portee`, `mtb_chien` : déclaration de la méta, case rendue **à la fin de `#misc-publishing-actions`** sur l'écran classique — donc sous la ligne « Publié le : », mais **au-dessus** du lien de corbeille et du bouton d'enregistrement, qui restent dans l'encadré, panneau `PluginPostStatusInfo` sur l'écran de blocs, sauvegarde, et l'avertissement propre à la page d'accueil. `show_in_rest` **vrai pour `page` seule** — seules les pages ont l'éditeur de blocs. | `init` **20** (`declarer_le_champ`, `declarer_le_script`) · `post_submitbox_misc_actions` 10 · `save_post_page`, `save_post_mtb_portee`, `save_post_mtb_chien` 10, **dérivés de `TYPES` par boucle et jamais écrits à la main** · `enqueue_block_editor_assets` 10. Constantes `TYPES` et `CHAMP` dans `declaration.php`. Aucune fonction globale. |
| `query/` | `chien` | #4 | Les deux fonctions de lecture de la fiche Chien : toutes les clés du retour sont toujours présentes même quand la donnée manque, les valeurs sont recopiées et jamais reformatées, et un groupe de statut sans chien n'est pas renvoyé. | 2 fonctions globales : `mtb_get_chien`, `mtb_get_chiens_par_statut`. **Aucun crochet.** |
| `query/` | `coordonnees` | #38 | L'option `mtb_core_coordonnees`, ses valeurs de départ et les trois fonctions de lecture des coordonnées de l'élevage : la valeur **stockée**, jamais une mise en forme, et `null` quand le réglage est vide. | 3 fonctions globales : `mtb_get_telephone_elevage`, `mtb_get_page_contact`, `mtb_get_coordonnees_elevage`. **Aucun crochet**, aucune lecture de base à l'inclusion. |
| `query/` | `mise-en-sommeil` | #52 | Module **frère** de `page-protegee`, jamais son extension : retire des moteurs, du plan du site, de la recherche interne **et des flux** tout contenu dont `_mtb_en_sommeil` vaut `'1'` — **sans mot de passe, et sans jamais toucher au lien direct**, qui continue de rendre 200. La méta a **trois** états : `'1'` endormi · `'0'` réveillé · **clé absente** = jamais réglé, distinction sans laquelle la conversion rendormirait ce que l'éleveuse a réveillé. `exclure_du_plan_du_site()` emploie `post__not_in` et **jamais** une `meta_query` : mesuré, la forme en `OR` aurait retiré du plan **tout contenu ne portant aucune ligne dans `wp_postmeta`, dont l'Accueil**, en silence. | `wp_robots` **20** · `wp_sitemaps_posts_query_args` 10 · `pre_get_posts` 10. Les rappels **s'empilent** avec ceux de `page-protegee` : chacun mute des clés, aucun ne remplace `$args`. Constantes `CLE`, `ENDORMI`, `REVEILLE` dans `etat.php` ; mémoïsation par requête et drapeau de ré-entrance. Aucune fonction globale. **Amendement déclaré dans `docs/contracts/issue-52.md` §5 et §11.3.** |
| `query/` | `page-protegee` | #23 | Exclut le contenu protégé par mot de passe du plan du site, de la recherche et des flux, et le déclare non indexable. Le test de contexte vit **dans le rappel**, jamais en garde de fichier. | `wp_sitemaps_posts_query_args` 10 · `pre_get_posts` 10 (recherche **et** flux) · `wp_robots` 20. Aucune fonction globale, aucune option, aucune méta. **Amendement au §2 déclaré dans `docs/contracts/issue-23.md` §2.3.** |
| `query/` | `portee` | #3 | Les six fonctions de lecture des portées. **C'est ici, et nulle part ailleurs, que se décide « la dernière portée »** : sur la date de naissance, jamais sur la date de saisie ; une portée non datée ne peut donc pas le devenir. | 6 fonctions globales : `mtb_get_portee`, `mtb_get_derniere_portee`, `mtb_get_portee_par_identifiant`, `mtb_get_portees`, `mtb_get_portees_du_chien`, `mtb_get_portee_voisine`. **Aucun crochet.** |
| `query/` | `resultat` | #5 | Les quatre fonctions de lecture des résultats de travail, dont **l'unique énumération close des disciplines** — clé stockée vers libellé imprimable — que ni l'écran de saisie, ni le tri, ni aucun composant ne refait. | 4 fonctions globales : `mtb_resultat_disciplines`, `mtb_resultat_sexes`, `mtb_get_resultats_travail_par_discipline`, `mtb_get_resultats_travail_du_chien`. **Aucun crochet.** |
| `blocks/` | `bandeau-alerte` | #9 | Le composant « Bandeau d'alerte » : un message court et éditable, rendu au serveur, sans panneau latéral ni réglage — l'aperçu de l'éditeur **est** le champ éditable. | Bloc `mtb/bandeau-alerte`, catégorie `mtb` · `init` 20 → poignée `mtb-bandeau-alerte-editeur` puis `register_block_type( __DIR__ )`. Aucune fonction globale. |
| `blocks/` | `bandeau-ouverture` | #6 | Le composant « Bandeau d'ouverture », **et l'arbitrage du `h1` unique de la page** : il efface le titre rendu par `core/post-title` dans les seuls cas où c'est lui qui porte le titre principal. | Bloc `mtb/bandeau-ouverture` · `init` 20 → poignée `mtb-bandeau-ouverture-editeur` · `render_block_core/post-title` 10 · fonction globale `mtb_bandeau_ouverture_porte_le_titre` (admise au §5 amendé le 2026-08-18). |
| `blocks/` | `categorie-mtb` | #6 | Déclare la catégorie « Mont Brabant » de l'insérteur, **en tête** de liste, une seule fois pour tout le catalogue ; les composants s'y raccrochent par le seul `"category": "mtb"` de leur `block.json`. Ce module n'enregistre aucun bloc et ne grossit jamais. | `block_categories_all` 10, avec garde d'idempotence. **Aucun bloc enregistré, aucune fonction globale, aucun `init`.** |
| `blocks/` | `coordonnees-plan` | #11 | Le composant « Coordonnées et plan d'accès ». `block.json` étant du JSON statique, aucune adresse n'y est recopiée : les valeurs par défaut du bloc sont **injectées à l'enregistrement** depuis les coordonnées lues par `query/coordonnees`. | Bloc `mtb/coordonnees-plan` · `init` 20 → poignée `mtb-coordonnees-plan-editeur` · `block_type_metadata` 10, **posé à l'inclusion** parce qu'il doit précéder `register_block_type_from_metadata()` · 3 fonctions globales : `mtb_coordonnees_plan_rendu`, `mtb_coordonnees_lien_telephone`, `mtb_coordonnees_lien_courriel`. |
| `blocks/` | `derniere-portee` | #12 | Le composant « Encart dernière portée ». Il **ne décide pas** de ce qu'est la dernière portée : il appelle `mtb_get_derniere_portee()` sous `function_exists()`. | Bloc `mtb/derniere-portee` · `init` 20 → poignée `mtb-derniere-portee-editeur`. Aucune fonction globale, aucun autre crochet. |
| `blocks/` | `encart-appel` | #10 | Le composant « Encart d'appel » : appel téléphonique et bouton vers la page de contact, plusieurs par page autorisés. Le numéro et la page viennent de `query/coordonnees` ; la validité de la page est éprouvée **au rendu**, jamais au réglage. | Bloc `mtb/encart-appel` · `init` 20 → poignée `mtb-encart-appel-editeur`. Aucune fonction globale, aucun autre crochet. |
| `blocks/` | `fiche-information` | #7 | Le composant « Fiche d'information », et le **même balisage** offert aux gabarits hors de tout contexte de bloc — les fiches Portée, Chien et Résultat n'acceptant aucun bloc. | Bloc `mtb/fiche-information` · `init` 20 → poignée `mtb-fiche-information-editeur` · fonction globale `mtb_fiche_information_balisage` (admise au §5 amendé le 2026-08-18). |
| `blocks/` | `formulaire-contact` | #22 | Le composant « Formulaire de contact » et **le traitement de sa soumission** : destination, jeton, assainissement et messages, sans aucun appel réseau ni cookie. L'écran d'édition reçoit l'**état** de la destination en un mot, jamais l'adresse de l'élevage. | Bloc `mtb/formulaire-contact` · `init` 20 → poignée `mtb-formulaire-contact-editeur` · `template_redirect` **1** · `wp_robots` → `wp_robots_no_robots`, posé **au rendu** par `entetes_sans_cache()` sur la confirmation comme sur toute réponse en erreur, jamais au chargement. **Amendement au §2 déclaré dans le module** (décision 46). Aucune fonction globale. |
| `blocks/` | `galerie-photos` | #8 | Le composant « Galerie photos » : une série de vignettes ouvrant chacune la photo en grand, à partir du seul attribut `photos`. Plusieurs galeries par page. | Bloc `mtb/galerie-photos` · `init` 20 → poignée de script `mtb-galerie-photos-editeur` **et poignée de style d'éditeur `mtb-galerie-photos-editeur-style`** (`editeur.css`, seule feuille de l'extension, motivée au contrat #8). Aucune fonction globale, aucun autre crochet. |
| `blocks/` | `grille-chiens` | #14 | Le composant « Grille de chiens », groupé par statut dans l'ordre gelé rendu par `mtb_get_chiens_par_statut()`, et le **même** rendu offert aux gabarits — un seul endroit où le balisage existe. | Bloc `mtb/grille-chiens` · `init` 20 → poignée `mtb-grille-chiens-editeur` + `wp_localize_script( 'mtbGrilleChiens' )` (tout le texte de l'éditeur vient du serveur) · fonction globale `mtb_grille_chiens_rendu` (admise au §5 amendé le 2026-08-18). |
| `blocks/` | `lien-de-recours` | #16 | Le composant « Lien de recours » des pages « introuvable » et « résultats de recherche » : un `<li>` dont l'adresse est **calculée** vers l'accueil, l'index des portées ou la meute, et qui **disparaît** si sa destination n'existe pas ou est protégée par mot de passe. **`"inserter": false`** — seul composant non offert à l'éleveuse, donc **sans fiche d'aide** ; il est posé par le thème dans `404.html`, `search.html` et `index.html`. | Bloc `mtb/lien-de-recours` (attribut `cible`, défaut `accueil` ; `usesContext: mtb/hors-liste`) · `init` 20 → poignée `mtb-lien-de-recours-editeur` · `render_block_context` 10 — seul moyen de constater l'absence de conteneur de liste, le cœur ne transmettant le bloc parent qu'aux filtres de rendu. Aucune fonction globale. |
| `blocks/` | `liste-portees` | #13 | Le composant « Liste de portées », qui se remplit lui-même par `mtb_get_portees()`, et transmet à l'éditeur — **et à lui seul** — la liste des années ayant au moins une portée. | Bloc `mtb/liste-portees` · `init` 20 → poignée `mtb-liste-portees-editeur` · `enqueue_block_editor_assets` 10 → `wp_localize_script( 'mtbListePorteesAnnees' )`, le seul crochet qui ne court que dans l'éditeur. Aucune fonction globale. |
| `blocks/` | `tableau-resultats` | #15 | Le composant « Tableau de résultats » — par discipline pour la page Travail, par chien pour un palmarès — et les **deux** rendus offerts aux gabarits. L'ordre n'est jamais réglable : discipline de l'année la plus récente à la plus ancienne, carrière de la première à la dernière. | Bloc `mtb/tableau-resultats` · `init` 20 → poignée `mtb-tableau-resultats-editeur` + `wp_localize_script( 'mtbTableauResultats' )` · 2 fonctions globales : `mtb_tableau_resultats_rendu`, `mtb_tableau_resultats_du_chien_rendu`. |
| `admin/` | `coordonnees` | #38 | L'écran unique « Coordonnées » — adresse, téléphone, courriel, page de contact — menu de **premier niveau** en position 24, juste après les trois types de contenu. Il **écrit** l'option et ne la lit pour personne d'autre : les fonctions de lecture vivent dans `query/coordonnees`. | `admin_menu` 10 → `add_menu_page` · `admin_post_mtb_enregistrer_coordonnees` 10. Nonce `mtb_coordonnees_nonce`, contrôle de capacité à l'affichage **et** à l'écriture. Aucune fonction globale, aucun bloc. |
| `admin/` | `corbeille` | #28 | Ajoute à la **seule** clé « untrashed » des trois types l'avertissement qui manque au bandeau du cœur : un contenu sorti de la corbeille revient en **brouillon**, donc hors du site. En priorité 20 il **complète** la phrase posée par `fields/`, sans jamais la recomposer. | `bulk_post_updated_messages` **20**, avec test `is_admin()` dans le rappel. Aucune fonction globale, aucun autre crochet. |
| `admin/` | `description-photo` | #35 | Renomme, dans la seule administration, l'étiquette « Texte alternatif » en « Description de la photo (pour les personnes aveugles) ». La comparaison porte sur la **chaîne source anglaise**, jamais sur sa traduction. Ne touche ni la valeur saisie, ni le site public. | `gettext` 10, sous garde `is_admin()` à l'inclusion. Aucune fonction globale, aucun autre crochet. |
| `admin/` | `listes` | #28 | **Un seul module pour les trois listes** : colonnes, ordre par défaut calculé en PHP, filtre déroulant à la place de la liste des mois, et champ « Disponibilité » dans la modification groupée des portées. Aucune colonne n'est rendue triable au clic ; le rappel sur les colonnes triables ne sert **qu'à retrancher** l'ordre initial de la colonne Date. | Sous garde `is_admin()` : `manage_mtb_{portee,chien,resultat}_posts_columns` 10 · `manage_edit-mtb_{portee,chien,resultat}_sortable_columns` 10 · `manage_mtb_{portee,chien,resultat}_posts_custom_column` 10 · `disable_months_dropdown` 10 · `restrict_manage_posts` 10 · `pre_get_posts` 10 · `bulk_edit_custom_box` 10 · `bulk_edit_posts` 10. **Écart au commentaire du chargeur déclaré dans le module, sans renvoi de contrat** — le chargeur annonce `admin_menu` et `admin_init` pour le groupe, ce sont ici d'autres crochets d'administration. Aucune fonction globale. |
| `admin/` | `medias` | #8 | Les deux réglages de traitement d'une photo **au téléversement** : la sous-taille `mtb-vignette-galerie` (400 px, non rognée, donc candidate de `srcset`) et la sortie WebP des seuls JPEG, le PNG restant intact pour ne pas dégrader un pedigree numérisé. Sans effet sur les fichiers déjà présents. | `init` 10 → `add_image_size( 'mtb-vignette-galerie' )` · `image_editor_output_format` 10. **Aucune garde `is_admin()`, délibérément** : la sous-taille doit exister sous WP-CLI et en REST, sans quoi le fichier ne serait jamais écrit. Aucune fonction globale. |
| `admin/` | `sommeil` | #52 | Ajoute la mention **« En sommeil »** à côté du titre, dans les listes d'administration. **La mention dérive de l'état, sans aucune liste de types** : elle resterait juste si un type futur portait la clé. Ne recopie **ni la clé, ni le libellé** : il inclut les **fichiers** `query/mise-en-sommeil/etat.php` et `fields/sommeil/libelles.php`, jamais le `bootstrap.php` d'un autre module, et ne dépend donc pas de l'ordre de parcours du chargeur. | `display_post_states` 10. Aucune fonction globale, aucune méta, aucune option, aucun autre crochet. |
| `migration/` | `import-fixtures` | #29 | La commande `wp mtb import-fixtures` : jeu de contenus **fictif** de la pile de développement, rejouable sans doublon, qui ne supprime jamais rien. Ses trois options sont facultatives ; c'est par `--help` que `provision.sh` sonde son existence. | Garde `WP_CLI` en première ligne exécutable, avant `ABSPATH` : sur une requête web, **rien** n'est exécuté au-delà. `WP_CLI::add_command( 'mtb import-fixtures' )` (`--chiens`, `--portees`, `--resultats`). Aucun crochet, aucune fonction globale. |
| `migration/` | `indexation-heritee` | #24 | **Ne pose plus aucune directive d'indexation depuis le 2026-09-07.** #52 a **converti** le fait hérité en un état que l'éleveuse pilote : `robots.php` est **supprimé** avec ses deux `add_filter`, et `ecarter_les_noindex()` est retirée de `plan-du-site.php`. Le module **conserve** le fait recopié — `_mtb_robots_source`, 5 contenus, valeur et provenance **intactes** (décision 55) — et le convertit en `_mtb_en_sommeil`, de façon **idempotente** et **sans jamais rendormir un contenu réveillé**. La liste des identifiants n'est **jamais** codée en dur : la conversion dérive de la méta. Conserve le retrait du fournisseur `users` du plan du site (exception motivée au §6.4, datée du 2026-09-05). **Depuis le 2026-09-08 (#50)**, répond lui-même le **404 franc** que le cœur ne pose pas pour un fournisseur retiré : `render_sitemaps()` sort par un `return` **nu** quand le fournisseur n'est pas au registre, laissant un faux 200 en HTML (dette T106) ; la constante `FOURNISSEURS_RETIRES` de `plan-du-site.php` est **l'unique écriture** du nom d'un fournisseur retiré et est lue par **les deux** rappels, si bien qu'il ne peut exister **ni retrait sans 404, ni 404 sans retrait**. **Depuis le 2026-09-08 (#49)**, **neutralise la requête d'auteur** dans `archives-d-auteur.php` : les clés `author` et `author_name` sont **retirées de la requête** — jamais une réponse posée après coup — et `error` est mis à `'404'`, si bien que `/author/<slug>/`, `/author/<slug>/feed/`, `?author=N` et `?author_name=<slug>` rendent **tous le même 404 que n'importe quelle adresse inexistante du site, sans `Location` distinct**. **L'oracle d'énumération est fermé SUR CES FORMES, et sur elles seules : la route REST `/wp-json/wp/v2/users` publie toujours la liste des deux comptes et le nom civil de l'éleveuse, et `dc:creator` des flux publie l'identifiant de connexion du compte 1 — résidus R1, R2, R4 et R6 du §9 de `docs/contracts/issue-49.md`, dette ouverte au board.** **Aucun identifiant en dur, aucun numéro de compte, aucun `user_nicename` n'entre dans le code de ce module** — les seules occurrences du dépôt sont des relevés datés, en commentaire — donc le rappel ne lit **aucun compte** et vaut pour les comptes **présents et futurs**. **Aucune règle de réécriture n'est touchée, aucun `flush` n'est requis, aucun état n'est laissé en base** (dette T105). | `mtb_core_mise_a_jour` 10 · `added_post_meta` 10 · `updated_post_meta` 10 · `admin_init` 10 · `wp_sitemaps_add_provider` 10 · **`template_redirect` 20** (#50, priorité choisie pour passer après `render_sitemaps()` **et** après `redirect_canonical`, dont le devineur de 404 mordrait sinon) · **`request` 10** (#49, choisi **contre** `template_redirect` 20 : #50 §D.1 a **mesuré** que `redirect_canonical` `exit` en priorité 10, donc un rappel à 20 **ne mordrait jamais** sur `?author=N`. **Ce filtre court AUSSI en administration** — `wp()` y est appelé par `wp_edit_posts_query()`, `wp-admin/includes/post.php:1319`, et par `wp_edit_attachments_query()`, `:1407` — d'où la garde `is_admin()` posée par `8b99687` : sans elle, l'onglet « À moi » d'une liste servait un 404 en affichant tous les contenus. La garde REST est `isset( $variables['rest_route'] )`, **jamais** `defined( 'REST_REQUEST' )`, qui n'est pas encore définie à cet instant ; **le tableau n'est jamais remplacé, seules des clés nommées sont retirées**). **Ne pose plus `wp_robots` ni `wp_sitemaps_posts_query_args`** : c'est `query/mise-en-sommeil` qui les porte désormais. Aucune garde `WP_CLI`, aucune commande, aucune fonction globale. **Amendement déclaré dans le module** (« le même amendement que le module voisin » `redirections-301`) ; **retraits déclarés dans `docs/contracts/issue-52.md` §6 et §9 (A2, A7)**, dont le §12 reconduit nommément l'interdit de retirer un rappel de `wp_robots` en croyant dédoublonner ; **relèves dans `docs/contracts/issue-50.md` §7 et `docs/contracts/issue-49.md` §13.1, §13.2 et son amendement daté du 2026-09-08**. |
| `migration/` | `portees-chiens` | #20 | Les deux commandes de reprise **réelle** des 17 fiches Chien et des 27 portées : l'import crée l'absent, laisse l'existant intact, ne supprime jamais, **refuse de tourner** si la base porte du contenu de démonstration, et lit les photographies sur le disque — aucun appel réseau. La vérification, en cinq passes, n'écrit rien et nomme chaque échec. | Garde `WP_CLI` en première ligne exécutable, avant `ABSPATH`. `WP_CLI::add_command( 'mtb importer-portees-chiens' )` · `WP_CLI::add_command( 'mtb verifier-portees-chiens' )` (`--chiens`, `--portees`, `--source`, `--photos`). Aucun crochet, aucune fonction globale. |
| `migration/` | `redirections-301` | #24 | Table gelée des 52 adresses de l'ancien site ; 301 sur `template_redirect` 1 ; réparation des ancres internes sur `the_content` 20 ; commande `wp mtb verifier-redirections`. Amendement au §2 déclaré dans `docs/contracts/issue-24.md`. Le module **n'écrit rien** — ni option, ni contenu, ni méta, ni fichier, ni réseau — et ne dépend d'aucun état en base pour se déclencher. **Depuis le 2026-09-08 (#53), sa commande porte une étape 6 bis qui mesure l'EFFET de la conversion du fait hérité, et non la présence de la clé** (dette T109 : l'étape 6 comptait `_mtb_robots_source` en base et restait **verte** même si `convertir()` n'avait jamais couru — muette sur toute base restaurée par copie). Verdict d'état par contenu sur la population « porte `_mtb_robots_source` **ET** `demande_noindex()` », liste réelle du plan du site produite par le cœur **en processus**, et **sonde de contraste** qui retire puis rétablit `MiseEnSommeil\exclure_du_plan_du_site` pour **attribuer** l'effet à ce rappel. **Attention en lisant « n'écrit rien » : cette sonde MUTE LA TABLE DES FILTRES EN MÉMOIRE**, le temps d'une exécution WP-CLI, rétablie en `finally` avec ré-assertion `has_filter()` imprimée — ce n'est pas une écriture, et une chaîne future qui découvrirait ce `remove_filter` ne doit pas y lire une violation. La doctrine du fichier passe de **deux à trois natures de contrôle** ; la balise `robots` et la recherche interne **restent hors de cette commande** et l'étape le déclare à chaque exécution. L'aide `--help` est accordée à ce que la commande mesure (extension d'empreinte, §13 de `docs/contracts/issue-53.md`). | `template_redirect` **1** · `the_content` **20**, tous deux **sans garde `WP_CLI`** — ce sont des services de front. Sous garde `WP_CLI` seulement : `WP_CLI::add_command( 'mtb verifier-redirections' )`. Aucune fonction globale. |
| `migration/` | `resultats-pages` | #21 | La commande `wp mtb reprise-resultats-pages` : reprise des résultats de travail et des pages libres de l'ancien site, avec ses correspondances de fiches chien, son mode `--simuler` qui n'écrit rien, son mode `--verifier` qui compare la base aux fichiers sans jamais écrire, et son mode `--raccrocher` qui ne pose que les liens chien restés à zéro. | Garde `WP_CLI` en première ligne exécutable, avant `ABSPATH`. `WP_CLI::add_command( 'mtb reprise-resultats-pages' )` (`--resultats`, `--pages`, `--correspondances`, `--photos`, `--simuler`, `--verifier`, `--raccrocher`). Aucun crochet, aucune fonction globale. |

> Dernière mise à jour : **clôture du lot 20, le 2026-09-08** — la ligne `migration/indexation-heritee`
> intègre le **404 franc du sous-plan retiré** (#50), la **neutralisation des archives d'auteur** (#49)
> et sa garde `is_admin()` ; la ligne `migration/redirections-301` intègre l'**étape 6 bis** de sa
> commande (#53) et l'avertissement sur la mutation en mémoire de la table des filtres.
> **Aucun module créé : 37 modules.** Les deux crochets de front ajoutés par ce lot — `template_redirect`
> 20 et `request` 10 — sont désormais au tableau ; ils y manquaient, et c'est ce tableau que la chaîne
> suivante lira pour détecter un crochet clandestin.
> Mise à jour précédente : clôture du lot 19, le 2026-09-07 — recensement complet des 34 modules
> depuis leur code par #51, puis ajout par le lead des **3 modules de #52** et réécriture de la ligne
> `migration/indexation-heritee`, que #52 a transformée. **37 modules.**
> Mise à jour antérieure : clôture du **lot 18** (issues #23, #24 — #42 n'a livré aucun module).
>
> **Note de solde — 2026-09-07 (#51).** L'avertissement daté du 2026-09-05 qui suit porte le compte
> **12 lignes / 34 modules / 22 manquantes**. C'est le compte **du 2026-09-05**, et **il est payé** :
> la table ci-dessus porte les 34 lignes, écrites depuis le code des 34 `bootstrap.php` et non depuis
> l'énoncé des issues qui les ont créés. L'avertissement n'est **ni effacé ni retouché** — il énonce la
> cause du trou de méthode, et un document ne se relit pas mieux quand on en efface ce qui l'a corrigé.
> Sa dernière phrase reste vraie : **#27 n'a livré aucun module**.
>
