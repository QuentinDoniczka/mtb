...
| **A5** | Les 3 photos, `docs/` n'étant monté dans aucun conteneur *(prémisse PÉRIMÉE — voir l'annotation)* | **Téléversement DIFFÉRÉ. Aucune duplication d'octets dans l'extension.** Les pages sont créées sans photo ; `--photos` sans défaut, avertissement bruyant nommant les trois fichiers et le montage manquant | Le plan back proposait de copier **1 125 604 octets** dans l'empreinte. Refusé : dupliquer une archive crée **une seconde source de vérité pour une photographie**, ce que la contrainte 3 interdit dans son esprit, et l'engage dans git pour toujours. Surtout, **le texte alternatif est déjà une question bloquante à l'éleveuse** : les photos ne peuvent de toute façon pas être reprises *correctement* aujourd'hui. Le montage `docs/:ro` dans `wpcli` est une ligne dans `compose.yaml`, hors empreinte. **Manque déclaré, pas case cochée.** L'état `photo_absente` est déjà géré : l'emplacement n'existe pas, D12 tenue. **— Annotation du 2026-08-28 : la prémisse du montage est PÉRIMÉE.** Le commit `01d4489`, du **même lot**, monte l'archive : `docs/` est atteignable depuis les conteneurs. **La décision A5 ne change pas pour autant, mais son motif se réduit à un seul** — le **texte alternatif**, absent de la source et impossible à rédiger sans savoir qui est sur la photo, donc question à l'éleveuse. Le refus de dupliquer 1 125 604 octets dans l'extension reste valable en soi. Cette annotation est portée ici parce que **deux autres endroits du même fichier déclaraient déjà la prémisse périmée pendant que celui-ci l'affirmait encore** : un contrat qui se contredit avec lui-même est pire qu'un contrat faux, on ne sait plus lequel de ses passages fait foi. |
...
| **A8** | Les 5 résultats de démonstration de `provision.sh` partagent la table | **L'importeur DÉTECTE et AVERTIT ; il ne supprime jamais rien** | `provision.sh:226-230` sème 5 résultats fictifs (marqueurs `demo-*`, affixe « de Démonstration ») dans le même type que mes 61 réels : `/travail/` **mélangera les deux jeux**, et l'ordre intra-année en dépend. Supprimer du contenu qui n'est pas le sien est hors de tout mandat. **Point de vérification pour la passe d'intégration du lot.** **— Annotation du 2026-08-28 : le remède que prescrivait ce paragraphe est PÉRIMÉ.** Il disait « le geste correct est `docker compose down -v` ». Il ne l'est pas : `provision.sh` **resème** le jeu de démonstration à chaque provisionnement, donc `down -v` rend une base à nouveau semée et le garde refuse de nouveau — l'opérateur tourne en rond. `resultats-pages/journal.php:368`, du **même lot**, déclare déjà ce remède insuffisant, et `portees-chiens/journal.php` a été corrigé dans le même sens. Le geste qui marche est de démarrer **sans** ce semis : réglage **`MTB_FIXTURES=0`**, livré par le commit `01d4489`. **La décision A8 elle-même ne change pas** — détecter et avertir, ne jamais supprimer. Annotation portée par la chaîne #20, #21 étant rendue, sur le mode déjà employé aux annexes A2 et A5 de ce fichier : **un contrat qui se contredit avec lui-même est pire qu'un contrat faux**, on ne sait plus lequel de ses passages fait foi. |
